...
export const DevPage: React.FC = () => {
  const sortedTimeline = [...devTimeline].sort((a, b) => {
  ...
  const firstEntry = sortedTimeline[0];
  const latestEntry = sortedTimeline[sortedTimeline.length - 1];
  const displayTimeline = [...sortedTimeline].reverse();
          ...
          <Card className="border border-border/60 bg-white/90">
            ...
            <CardContent className="space-y-3 text-sm leading-7 text-muted-foreground">
              <div className="rounded-2xl bg-amber-50 px-4 py-3 text-amber-900">
                DEV 页面现在收敛为纯时间线视图，RAG、loop、前端和评估改动都直接回填到对应时间点，不再单独拆 block。
              </div>
              <div className="rounded-2xl bg-amber-50 px-4 py-3 text-amber-900">
                当前维护重点仍然是记录“结构变化、链路打通、评估口径变化、前端可见变化”，避免把 DEV 页面写成零散待办板。
              </div>
            </CardContent>
          </Card>
        </div>

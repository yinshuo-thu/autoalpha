...
import { Activity, BrainCircuit, FlaskConical, FolderSync, Sparkles, Wand2 } from 'lucide-react';
...
interface LoopStatus {
  ...
  live_state?: {
    ...
    last_metrics?: Record<string, number | boolean>;
...
function buildPnlDrawdownSeries(
  cumulative?: ModelLabPoint[],
  drawdown?: ModelLabPoint[],
  daily?: ModelLabPoint[],
): PnlDrawdownPoint[] {
  const drawdownByDate = new Map((drawdown || []).map((point) => [point.date, Number(point.value) || 0]));
  const dailyByDate = new Map((daily || []).map((point) => [point.date, Number(point.value) || 0]));
  return (cumulative || [])
    .map((point, index, rows) => {
      const pnl = Number(point.value) || 0;
      const previousPnl = index > 0 ? Number(rows[index - 1]?.value) || 0 : 0;
      return {
        date: point.date,
        pnl,
        dailyPnl: dailyByDate.get(point.date) ?? (index === 0 ? pnl : pnl - previousPnl),
        drawdown: drawdownByDate.get(point.date) ?? 0,
      };
    })
    .filter((point) => Number.isFinite(point.pnl) && Number.isFinite(point.dailyPnl) && Number.isFinite(point.drawdown));
}
...
const PnlDrawdownChart = ({
  ...
  const winRate = data.length ? (data.filter((point) => point.dailyPnl > 0).length / data.length) * 100 : 0;
  const worstDay = data.length ? Math.min(...data.map((point) => point.dailyPnl)) : 0;
  const bestDay = data.length ? Math.max(...data.map((point) => point.dailyPnl)) : 0;
  const avgDaily = data.length ? data.reduce((sum, point) => sum + point.dailyPnl, 0) / data.length : 0;
  return (
    <div className="rounded-2xl bg-slate-50 p-3">
      ...
      {data.length ? (
        <div className="mb-2 grid grid-cols-2 gap-2 text-[11px] text-slate-600 sm:grid-cols-4">
          <div className="rounded-xl bg-white/80 px-2 py-1.5">日胜率 <span className="font-mono text-slate-900">{formatNumber(winRate, 1)}%</span></div>
          <div className="rounded-xl bg-white/80 px-2 py-1.5">平均日 PnL <span className="font-mono text-slate-900">{formatNumber(avgDaily, 5)}</span></div>
          <div className="rounded-xl bg-white/80 px-2 py-1.5">最佳日 <span className="font-mono text-emerald-700">{formatNumber(bestDay, 5)}</span></div>
          <div className="rounded-xl bg-white/80 px-2 py-1.5">最差日 <span className="font-mono text-rose-700">{formatNumber(worstDay, 5)}</span></div>
        </div>
      ) : null}
      {data.length ? (
        <div className={heightClass}>
          <ResponsiveContainer width="100%" height="100%">
            <ComposedChart data={data} margin={{ top: 8, right: 8, bottom: 4, left: 0 }} barCategoryGap="12%">
              <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" />
              <XAxis dataKey="date" tick={{ fill: '#64748b', fontSize: 10 }} tickFormatter={formatAxisTime} minTickGap={26} />
              <YAxis yAxisId="pnl" tick={{ fill: '#64748b', fontSize: 10 }} tickFormatter={(value) => formatNumber(Number(value), 2)} />
              <YAxis yAxisId="daily" orientation="right" tick={{ fill: '#64748b', fontSize: 10 }} tickFormatter={(value) => formatNumber(Number(value), 3)} />
              <YAxis yAxisId="dd" orientation="right" hide domain={['auto', 0]} />
              <Tooltip
                formatter={(value: number, name: string) => [formatNumber(Number(value), 5), name]}
                labelFormatter={(label) => `交易日 ${label}`}
              />
              <Legend wrapperStyle={{ fontSize: 11 }} />
              <ReferenceLine yAxisId="pnl" y={0} stroke="#94a3b8" strokeDasharray="3 3" />
              <ReferenceLine yAxisId="daily" y={0} stroke="#94a3b8" strokeDasharray="3 3" />
              <Bar yAxisId="daily" dataKey="dailyPnl" name="每日 PnL" fill="#94a3b8" opacity={0.42} radius={[2, 2, 0, 0]} />
              <Area yAxisId="dd" type="linear" dataKey="drawdown" name="Drawdown" stroke="#ef4444" fill="#fecaca" fillOpacity={0.45} dot={false} />
              <Line yAxisId="pnl" type="linear" dataKey="pnl" name="累计 PnL" stroke={pnlColor} strokeWidth={2.5} dot={false} activeDot={{ r: 3 }} />
            </ComposedChart>
          </ResponsiveContainer>
        </div>
      ) : (
...
function livePhaseLabel(phase?: string) {
  ...
  return labels[phase || ''] || phase || '等待';
}

const LiveMiningPanel = ({ status }: { status: LoopStatus | null }) => {
  const live = status?.live_state;
  const metrics = live?.last_metrics ?? {};
  const metricItems = ([
    ['IC', metrics.IC],
    ['IR', metrics.IR],
    ['TVR', metrics.TVR],
    ['Score', metrics.Score],
  ] as Array<[string, number | boolean | undefined]>).filter(
    (item): item is [string, number] => typeof item[1] === 'number'
  );

  return (
    <div className="mt-4 rounded-3xl border border-emerald-200/70 bg-emerald-50/70 p-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex min-w-0 items-center gap-2">
          <Activity className={`h-4 w-4 ${status?.is_running ? 'text-emerald-600' : 'text-slate-400'}`} />
          <div className="min-w-0">
            <div className="text-sm font-semibold text-slate-900">实时研究进程</div>
            <div className="mt-1 break-words text-xs text-slate-600">{live?.message || '等待挖掘进程输出'}</div>
          </div>
        </div>
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-4">
        <div className="rounded-2xl bg-white/80 p-3">
          <div className="text-[11px] uppercase tracking-[0.16em] text-slate-500">Round / Idea</div>
          <div className="mt-1 text-sm font-semibold text-slate-900">
            {live?.round_label || '--'} {live?.idea ? ` · idea ${live.idea}/${live.ideas_total || '?'}` : ''}
          </div>
        </div>
        <div className="rounded-2xl bg-white/80 p-3">
          <div className="text-[11px] uppercase tracking-[0.16em] text-slate-500">Process</div>
          <div className="mt-1 text-sm font-semibold text-slate-900">
            {status?.pid ? `PID ${status.pid}` : '--'} · {status?.process_source || 'none'}
          </div>
        </div>
        <div className="rounded-2xl bg-white/80 p-3">
          <div className="text-[11px] uppercase tracking-[0.16em] text-slate-500">Discovery</div>
          <div className="mt-1 line-clamp-2 text-xs font-medium text-slate-800">
            {live?.eval_window || live?.data_window || '--'}
          </div>
        </div>
        <div className="rounded-2xl bg-white/80 p-3">
          <div className="text-[11px] uppercase tracking-[0.16em] text-slate-500">OOS</div>
          <div className="mt-1 line-clamp-2 text-xs font-medium text-slate-800">
            {live?.oos_window || '--'}
          </div>
        </div>
      ...
      {live?.data_window || live?.screen_window || live?.export_window ? (
        <div className="mt-3 grid gap-3 md:grid-cols-3">
          <div className="rounded-2xl bg-white/70 p-3">
            <div className="text-[11px] text-slate-500">Data</div>
            <div className="mt-1 line-clamp-2 text-xs font-medium text-slate-800">{live?.data_window || '--'}</div>
          </div>
          <div className="rounded-2xl bg-white/70 p-3">
            <div className="text-[11px] text-slate-500">Screen</div>
            <div className="mt-1 line-clamp-2 text-xs font-medium text-slate-800">{live?.screen_window || '--'}</div>
          </div>
          <div className="rounded-2xl bg-white/70 p-3">
            <div className="text-[11px] text-slate-500">Export</div>
            <div className="mt-1 line-clamp-2 text-xs font-medium text-slate-800">{live?.export_window || '--'}</div>
          </div>
        </div>
      ) : null}
      ...
      {typeof live?.progress === 'number' ? (
        <div className="mt-3 rounded-2xl bg-white/80 p-3">
          <div className="flex items-center justify-between gap-3 text-xs text-slate-600">
            <span>缓存/计算进度 {formatNumber(live.progress, 1)}%</span>
            <span>
              {live.progress_current ?? '--'}/{live.progress_total ?? '--'}
              {typeof live.eta_seconds === 'number' ? ` · ETA ${Math.ceil(live.eta_seconds / 60)} 分钟` : ''}
            </span>
          </div>
          <div className="mt-2 h-2 overflow-hidden rounded-full bg-slate-200">
            <div className="h-full rounded-full bg-emerald-500 transition-all" style={{ width: `${Math.min(100, Math.max(0, live.progress))}%` }} />
          </div>
        </div>
      ) : null}
          ...
          {metricItems.map(([label, value]) => (
            <div key={label} className="rounded-2xl bg-white/80 p-3">
              ...
              <div className="mt-1 font-mono text-sm font-semibold text-slate-900">
                {typeof value === 'number' ? formatNumber(value, label === 'TVR' || label === 'Score' ? 2 : 4) : '--'}
              </div>
            </div>
          ))}
        </div>
      ...
      {live?.current_formula ? (
        <div className="mt-3 rounded-2xl bg-white/80 p-3">
          <div className="text-[11px] uppercase tracking-[0.16em] text-slate-500">Current Formula</div>
          <div className="mt-1 break-all font-mono text-xs leading-5 text-slate-800">{live.current_formula}</div>
        </div>
      ) : null}

      {live?.last_recall || live?.last_rejection ? (
        <div className="mt-3 space-y-1 rounded-2xl bg-white/70 p-3 text-xs text-slate-600">
          {live.last_recall ? <div className="break-words">{live.last_recall}</div> : null}
          {live.last_rejection ? <div className="break-words text-rose-600">{live.last_rejection}</div> : null}
        </div>
      ) : null}
    </div>
  );
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="max-h-[90vh] w-full max-w-4xl overflow-y-auto rounded-3xl border border-border/60 bg-white p-6 shadow-2xl">
        ...
        <div className="rounded-2xl bg-slate-50 p-3">
          ...
          <div className="mb-3">
            <label className="text-[10px] uppercase text-muted-foreground">备注（IC 口径说明）</label>
            <div className="mt-1 text-[11px] leading-5 text-slate-500">
              项目内涉及到的 IC 均按实际 IC × 100 展示；例如展示 IC=0.85 代表实际 IC=0.0085。
            </div>
            <textarea
...
export const AutoAlphaPage: React.FC = () => {
  ...
  const [comboModal, setComboModal] = useState<{ modelName: string } | null>(null);
  ...
  const bestComboLongShortPnlSeries = buildPnlDrawdownSeries(
    ...
    bestComboSummary?.payload.daily_pnl_curve,
  );
  const bestComboLongOnlyPnlSeries = buildPnlDrawdownSeries(
    ...
    bestComboSummary?.payload.daily_long_pnl_curve,
  );
  ...
  return (
    <PageErrorBoundary>
      ...
      {balance?.warnings?.length ? (
	              ...
	              <div className="flex flex-wrap items-start justify-between gap-3">
	                <div className="min-w-0">
	                  ...
	                  <div className="mt-1 truncate text-sm font-semibold text-foreground">
		                ...
		                {bestComboSummary ? (
		                  <button
		                    onClick={() => setComboModal({ modelName: bestComboSummary.model })}
		              ...
		              {bestComboSummary ? (
		                <>
		                  ...
		                  <div className="mt-4 space-y-3 rounded-2xl bg-white/80 p-3">
		                    <PnlDrawdownChart title="2024 OOS 多空 PnL + 每日 PnL + Max DD" data={bestComboLongShortPnlSeries} pnlColor="#2563eb" heightClass="h-[260px]" />
		                    <PnlDrawdownChart title="2024 OOS 纯多头 PnL + 每日 PnL + Max DD" data={bestComboLongOnlyPnlSeries} pnlColor="#0f766e" heightClass="h-[260px]" />
		                  </div>
			                </>
			              ) : (
			                <div className="mt-3 text-sm text-muted-foreground">Model Lab 产出后会自动展示全因子中 Score 最高的 2024 OOS combo。</div>
			              )}
	            </div>
	          </div>
	        </Panel>
        ...
        <Panel title="OOS Combo Lab" subtitle="使用全部有效因子，固定以 2022-2023 为训练期、2024 为模拟 OOS 期，只保留真实 OOS 阶段也能实现的 combo 方法，并展示 2024 的 OOS Score / IC / IR / TVR。">
          <div className="grid min-w-0 gap-5">
            <div className="flex max-h-[620px] min-w-0 flex-col rounded-3xl border border-border/50 bg-white/90 p-4">
	              <div className="mb-3 flex items-center gap-2 text-sm font-medium text-foreground">
	                ...
	                {visibleModelEntries.map(([modelName, payload]) => {
	                  ...
	                  const groupMeta = getMethodGroupMeta(modelName);
		                  return (
		                    <button
		                      key={modelName}
		                      onClick={() => setComboModal({ modelName })}

...
export const devTimeline: DevTimelineEntry[] = [
  {
    ...
    title: 'v3 与 v2 彻底隔离并重建启动边界',
    summary: '按“不要再改 v2”的要求，把后续实现限定到 /Volumes/T7/autoalpha_v3，并把 v2 迁移种子、systematic 种子和 v3 在线挖掘结果分开记录。',
    tags: ['AutoAlpha v3', 'Migration', 'Runtime Boundary'],
    bullets: [
      '回退/停止继续污染 v2 的修改路径，后续代码、日志、output、knowledge、frontend 均只指向 /Volumes/T7/autoalpha_v3。',
      'v3 初始高通过率来自 v3sys systematic 种子和 v3v2 manual/v2 迁移种子，后续 autoalpha_* 才是在线 LLM-RAG 新挖掘分布。',
      'run_v3_watchdog.sh 默认使用 AUTOALPHA_V3_DAYS=0、target_valid=100，启动目标改为长期挖掘有效因子到 100 个。',
      '服务标签与前端入口统一为 v3，避免 v2/v3 混用造成日志、端口和输出目录误判。',
    ],
  },
  {
    timestamp: '2026-04-27T12:10:00+08:00',
    title: 'v3 RAG Phase 1：机制卡、结构化失败记忆与检索追踪',
    summary: '把原 v2 偏文本拼接的 RAG 升级为可追踪、可检索、可回灌的机制级记忆系统，为后续因子生成提供更细粒度上下文。',
    tags: ['RAG', 'Inspiration', 'Failure Memory', 'Trace'],
    bullets: [
      'inspiration_db.py 为 inspiration 建立 mechanism card 和 embedding，把灵感从纯文本素材升级为可语义检索的机制单元。',
      'knowledge_base.py 增加 passing factor empirical correlation rerank，把正样本召回从“语义相似”扩展到“语义 + 实证低相关/高贡献”。',
      'failure memory 改成结构化记录，替代单纯 exhausted skeleton，保留失败阶段、指标、gate 细节、公式结构和可回灌负样本。',
      'pipeline / llm_client 每次生成记录 retrieval trace，保留 query、召回来源、rerank 过程和最终 prompt section 组成。',
    ],
  },
  {
    timestamp: '2026-04-27T13:20:00+08:00',
    title: 'v3 RAG Phase 2：多查询召回、RRF/MMR 与探索位',
    summary: '在 Phase 1 记忆结构上继续增强搜索质量，让 LLM 不是只围绕最近高分父代微调，而是同时约束机制、实现、负例和新颖性。',
    tags: ['RAG', 'Retrieval', 'Bandit', 'Exploration'],
    bullets: [
      'llm_client / knowledge_base 支持 mechanism、implementation、negative、novelty 多 query 检索，并把各路结果合并给生成 prompt。',
      '召回后增加 RRF + MMR rerank，兼顾多路一致性、低冗余和与已有 passing 因子的差异度。',
      '新增 archetype/source/operator bandit，把不同灵感来源、结构类型、操作符族的历史产出反馈到采样权重。',
      '加入 exploration slot，强制保留低相关、高新颖度候选，降低搜索只围绕高分旧结构局部收敛的风险。',
    ],
  },
  {
    timestamp: '2026-04-27T15:50:00+08:00',
    title: '发现窗口恢复为 2022-2023，2024 仅作 OOS 报告',
    summary: '修正早期 days=180 导致窗口误解的问题，恢复当前研究设定：2022-2023 用于发现和 gate，2024 只展示 OOS 指标，不参与反馈。',
    tags: ['Evaluator', 'OOS', 'Data Window', 'Leakage Guard'],
    bullets: [
      'pipeline.py 明确 DISCOVERY_START/END=2022-2023，OOS_START/END=2024；days=0 表示完整 discovery，不再解释为 180 日。',
      'prepare_data.py 支持 exact cache 与 superset parquet date-filter loading，避免没有精确窗口缓存时反复重建数据。',
      '日志输出改为 Discovery window、Screen window、OOS report-only window、Export parquet window 四段，前端可直接展示研究窗口。',
      '2024 OOS metrics 标注 report-only / used_for_feedback=false，防止误把 2024 指标用于父代选择、PassGates 或 target-valid 停止条件。',
    ],
  },
  {
    timestamp: '2026-04-27T18:20:00+08:00',
    title: 'v3 前端记录页和实时研究进程修复',
    summary: '修复 AutoAlpha Record / 运行页在 v3 目录下读取错误、通过率缺失和研究进程不可见的问题，让后端日志和 knowledge 状态实时回到前端。',
    tags: ['Frontend', 'Records', 'Runtime Status', 'API'],
    bullets: [
      'server.py live status 解析 data/eval/screen/oos/export 窗口、round、parents、screen/full/OOS/export 事件，并返回给 AutoAlphaPage。',
      'AutoAlphaRecordsPage 修复 JSON endpoint 被 HTML fallback 污染导致的 Unexpected token < 错误。',
      '记录页补充因子通过率、source 转化率、passing 数、tested 数和有效因子贡献统计。',
      'factor correlation polling 和 source analysis 读取 v3 knowledge/research 输出，避免显示“暂无灵感源统计”或空代际数据。',
    ],
  },
  {
    timestamp: '2026-04-27T19:30:00+08:00',
    title: 'OOS Combo 图表细化与 SQLite idea cache 锁修复',
    summary: '针对前端 OOS 曲线过平和后台 idea_cache database locked 报错，分别修复图表粒度和 SQLite 并发写入策略。',
    tags: ['Frontend', 'Model Lab', 'SQLite', 'IdeaCache'],
    bullets: [
      'rolling_model_lab / server 不再把 2024 OOS 曲线压缩到 96 点，前端改为展示 242 个交易日的累计 PnL、daily PnL bar 和 drawdown area。',
      'AutoAlphaPage 的“全因子最佳 combo：2024 OOS PnL / Max DD”改为日级细图，避免长时间看起来过平。',
      'idea_cache.py 为 SQLite 增加 timeout、busy_timeout、WAL、synchronous=NORMAL，并对 push/pop/register/record/prune 增加 locked retry。',
      '后台 fill_worker 捕获 push/prune 失败，避免 database locked 直接杀死 idea-cache-fill 线程。',
    ],
  },
  {
    timestamp: '2026-04-27T20:45:00+08:00',
    title: '计算链路加速：无效因子不导出与 screen promotion 收紧',
    summary: '定位到 screen 勉强通过但 full gate 失败的候选仍在跑 OOS/full export 并写 200MB 级 parquet，改为只记录轻量失败记忆。',
    tags: ['Performance', 'Pipeline', 'Artifacts', 'Screening'],
    bullets: [
      'full eval 失败后直接写 screened_out 轻量记录，不再跑 2024 OOS、不再重算 2022-2024 full export、不再导出 parquet/research。',
      'artifact materialization 默认只允许 PassGates=true；AUTOALPHA_EXPORT_PROMISING_NONPASS=0 防止无效候选污染 output。',
      'screen promotion gate 新增 AUTOALPHA_SCREEN_PROMOTE_MIN_IC/IR/MAX_TVR，默认 0.45 / 2.0 / 360，减少弱候选进入 full eval。',
      'formula_parser 修复 x-0.5 被 tokenizer 当成负数导致 parse fail 的问题，提高 LLM 生成公式有效率。',
    ],
  },
  {
    timestamp: '2026-04-27T21:35:00+08:00',
    title: 'v3 长循环性能缓存与 confirmation window',
    summary: '继续优化当前在线挖掘的慢路径：缓存 DataHub、评估宽表和 pv 子窗口，并在 full eval 前增加非重叠确认窗口过滤 recent false positive。',
    tags: ['Performance', 'Cache', 'Confirmation', 'Evaluator'],
    bullets: [
      'pipeline.py 增加进程内 DataHub cache，后续 round 复用已加载的 2022-2024 数据，减少每轮 6460 万行 parquet 重读。',
      'evaluate_alpha 增加 resp/trading_restriction 宽表缓存，screen、confirmation、discovery 窗口按 key 复用 unstack 后结果。',
      'compute_alpha 增加 pv trading days 和小窗口 pv slice 缓存，screen/confirmation 复用日期切片；全窗口过大则不缓存以控制内存。',
      'screen 通过后先跑 early discovery confirmation window，默认 120 日、IC>=0.25、IR>=1.2、TVR<=380，确认不过就跳过 full 484 日计算。',
      '方向翻转从“IC<0 就翻”改为同时评估原始/翻转，并按 PassGates、Score、IC、IR 择优，避免交易限制导致翻转后更差。',
    ],
  },
  {
    timestamp: '2026-04-26T22:05:00+08:00',
    title: 'README 重写、发布工作区清理与 GitHub main 推送',
    summary: '把当前 AutoAlpha v3 的展示定位、最新 Combo/Fusion 结果、运行方式和不包含原始 parquet 的发布边界写入 README；同步发布工作区并推送到 GitHub main。',
    tags: ['Docs', 'GitHub', 'Release', 'Cleanup'],
    bullets: [
      'README 更新为当前完整项目说明，补充 deployed showcase、display-only 目录、最新 full-factor / fusion 结果。',
  ...
  },
  {
    ...
    summary: '把原先不完整的 OOS Combo Lab 扩展为完整多方法实验区，新增 TVR、方法卡与 Train/Val/OOS 对比。',
    tags: ['OOS Combo Lab', 'TVR', 'Frontend', 'Experiment'],
    bullets: [
      '补齐全因子组合的经典 Rank、Linear、Ridge、Tree、LightGBM、MLP、Transformer 等方法输出。',
      '所有 Combo 方案统一计算 TVR，并在摘要、卡片、方法对比和 OOS 指标中展示。',
      '点击方法可打开 Combo Card，查看方法语言描述、泄漏防护说明、Train/Val 曲线以及 Train/Val/OOS 指标对比。',
      'OOS Combo Lab 改名为 Exploratory OOS Combo Lab，并扩展为整行布局，强化多方法 OOS 指标对比。',
    ],
  },
  {
    timestamp: '2026-04-25T12:00:00+08:00',
    title: 'v2 独立迁移与 display-only 展示目录',
    summary: '从 /Volumes/T7/Scientech/autoalpha_v3 迁移出独立项目目录，并整理出不含原始 parquet 的展示专用目录用于 autoalpha.cn/v3。',
    tags: ['Migration', 'Display', 'Deployment', 'Data Boundary'],
    bullets: [
      '建立 /Volumes/T7/autoalpha_v3，迁移代码、计算结果和展示所需数据，消除对 /Volumes/T7/Scientech 下文件的运行依赖。',
      '建立 /Volumes/T7/autoalpha_v3_display，保留前端展示、压缩 JSON、静态资源和必要 submit/metadata 结果，不包含原始 parquet 数据集。',
      '优化 server compact API，限制大曲线点数并压缩 dashboard payload，降低公开展示页加载压力。',
      '将 https://autoalpha.cn/v3 指向展示目录服务，后续计算目录与展示目录通过同步流程解耦。',
    ],
  },
  {
  ...
  },
  {
    ...
    title: '前端雏形与首轮代码审查',
    summary: '建立 frontend 工程骨架，并开始对 AutoAlpha v3 做代码级与架构级检查。',
  ...
  },
  {
    ...
    tags: ['AutoAlpha v3', 'Refactor', 'Server'],
    bullets: [
      '提交 f16a81c，创建 clean AutoAlpha v3 package。',
  ...
  },
  {
    ...
    bullets: [
      ...
      '明确记录 AutoAlpha v3、LLM 挖掘、评估与前端/API 的持续增强方向。',
  ...
  },
  {
    ...
    bullets: [
      ...
      '将 autoalpha_v3 全量重构、前端组件、后端 API、leaderboard、评估器等累积修改一次性 commit 并 push。',

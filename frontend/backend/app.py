...
@app.get("/")
async def root():
    return {"message": "AutoAlpha API", "version": "3.0.0"}
...
async def browse_inspirations(
    ...
    try:
        from autoalpha_v3.inspiration_db import list_inspirations_paginated
...
async def add_inspiration(req: InspirationAddRequest):
    ...
    try:
        from autoalpha_v3.inspiration_db import prepare_inspiration, save_inspiration
...
async def sync_inspirations():
    ...
    try:
        from autoalpha_v3.inspiration_db import sync_prompt_directory, list_recent_inspirations, compose_inspiration_context, DB_PATH, PROMPT_DIR
...
async def toggle_inspiration(entry_id: int):
    ...
    try:
        from autoalpha_v3.inspiration_db import toggle_inspiration_status
...
async def delete_inspiration(entry_id: int):
    ...
    try:
        from autoalpha_v3.inspiration_db import delete_inspiration as _delete
        ok = _delete(entry_id)
        if not ok:
...
async def autoalpha_factor_correlations():
    """Return cached AutoAlpha v3 factor-correlation heatmap data."""
    try:
        from autoalpha_v3.factor_research import load_factor_correlation_cache

        return {"success": True, "data": load_factor_correlation_cache()}
    except Exception as exc:
        cache_path = PROJECT_ROOT / "autoalpha_v3" / "factor_correlations.json"

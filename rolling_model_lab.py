"""
autoalpha_v3/rolling_model_lab.py
...
V3_CURATED_MODEL_NAMES = [
    # v2-effective rank/weight combos: keep a compact, diverse subset.
    "TrainICIRRankCombo",
    "ValPowerRankCombo",
    "DiversifiedICRankCombo",
    "VolBalancedTopRankCombo",
    # Regression baselines that were consistently competitive in v2.
    "RidgeZScoreMetaModel",
    "TemporalRidgeLagMetaModel",
    # ML / neural / stack models: this group dominated v2, so keep more capacity here.
    "LightGBMMetaModel",
...
from autoalpha_v3.pipeline import run as run_pipeline
...
def _factor_full_coverage_days(factor: Dict[str, Any]) -> int:
    full = factor.get("full_2022_2024") if isinstance(factor.get("full_2022_2024"), dict) else {}
    try:
        return int((full or {}).get("days", 0) or 0)
    except Exception:
        return 0


def _list_valid_autoalpha_factors(min_coverage_days: int = 700) -> List[Dict[str, Any]]:
    factors = []
    for factor in kb.list_valid_factors(min_eval_days=0):
        parquet_path = factor.get("parquet_path") or ""
        coverage_days = max(
            int(factor.get("eval_days", 0) or 0),
            _factor_full_coverage_days(factor),
        )
        if (
            factor.get("status") == "ok"
            and parquet_path
            and Path(parquet_path).exists()
            and coverage_days >= int(min_coverage_days)
        ):
...
def _model_spec_items() -> list[tuple[str, Any, int]]:
    ...
    elif str(os.environ.get("AUTOALPHA_MODEL_LAB_FULL_SWEEP", "")).lower() not in {"1", "true", "yes", "on"}:
        wanted = set(V3_CURATED_MODEL_NAMES)
        items = [item for item in items if item[0] in wanted]
    return items
...
def _export_precomputed_oos_model_factor(
    *,
    ...
    if export_suffix:
        export_id = f"{export_id}_{export_suffix}"
    oos_export_dir = run_dir / "oos_exports"
    oos_export_dir.mkdir(parents=True, exist_ok=True)
    out_path = oos_export_dir / f"{export_id}.pq"
    sanity_report = _write_submit_ready_daily_alpha(daily_alpha, out_path)
    ...
    metadata_path = oos_export_dir / f"{export_id}_metadata.json"
    official_like_path = oos_export_dir / f"{export_id}_official_like_result.json"
    metadata_path.write_text(json.dumps(metadata, ensure_ascii=False, indent=2), encoding="utf-8")
    official_like_path.write_text(
        json.dumps([snapshot["result_preview"]] if snapshot["result_preview"] else [], ensure_ascii=False, indent=2),
        encoding="utf-8",

...
- 只有通过所有 gate 的因子才会复制到 `autoalpha_v3/submit`，并在研究 UI 中显示
  为可提交卡片链接。
...
|------|-----------------|--------------|
| 研究模式 | 脚本化实验：`research_loop.py`、`evaluate_alpha.py`、手动配置和 leaderboard 更新。 | 产品化循环：`autoalpha_v3/run.py` / `loop.py` 负责生成、筛选、导出、报告、通知和持久化。 |
...
| 提交安全 | 提交工具可以独立调用。 | 提交就绪是 gate 控制状态，只有 `PassGates=true` 因子会进入 `autoalpha_v3/submit`。 |
...
8. 通过因子会复制到 `autoalpha_v3/submit`，附带 metadata 和类官方结果 JSON；
...
- 备份 `autoalpha_v3/knowledge.json`；
- 归档旧 `autoalpha_v3/submit` 内容；
- 从源数据重算每个公式；
- 重新生成 `autoalpha_v3/output/<run_id>.pq`；
- 刷新 `knowledge.json` 中的指标和 gate 字段；
- 只把仍然通过 gate 的因子复制到 `autoalpha_v3/submit`；
- 在 `autoalpha_v3/recompute_reports` 下写出批处理摘要。

可提交因子是 `autoalpha_v3/submit` 目录下的 `.pq` 文件。每个文件都有对应 metadata JSON

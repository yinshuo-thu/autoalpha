"""
autoalpha_v3/knowledge_base.py
...
CORRELATIONS_PATH = Path(__file__).resolve().parent / "factor_correlations.json"
...
_CORR_CACHE: Dict[str, Any] | None = None
...
def _load_corr_lookup() -> dict[tuple[str, str], float]:
    global _CORR_CACHE
    if _CORR_CACHE is None:
        try:
            _CORR_CACHE = json.loads(CORRELATIONS_PATH.read_text(encoding="utf-8")) if CORRELATIONS_PATH.is_file() else {}
        except Exception:
            _CORR_CACHE = {}
    run_ids = [str(x) for x in (_CORR_CACHE.get("run_ids") or [])]
    ...
    for i, rid_i in enumerate(run_ids):
        ...
        for j, rid_j in enumerate(run_ids):
            if j >= len(row):
                continue
            try:
                lookup[(rid_i, rid_j)] = abs(float(row[j]))
            except Exception:
                pass
    return lookup


def _max_abs_corr(run_id: str, other_run_ids: list[str], lookup: dict[tuple[str, str], float]) -> float:
    vals = [lookup.get((run_id, other), lookup.get((other, run_id), 0.0)) for other in other_run_ids if other and other != run_id]
...
def _select_query_guided_rows(
    ...
    *,
    ...
    corr_lookup = _load_corr_lookup()
    parent_run_ids = [str(x) for x in (parent_run_ids or []) if x]
    ...
    def _mmr_rank(rows: list[tuple[float, Dict[str, Any], float | None]]) -> list[tuple[float, Dict[str, Any], float | None]]:
        remaining = list(rows)
        ordered: list[tuple[float, Dict[str, Any], float | None]] = []
        while remaining and len(ordered) < max_factors * 3:
            selected_ids = [str(item.get("run_id", "")) for _, item, _ in ordered]
            best_idx = 0
            best_score = float("-inf")
            for idx, (score, item, disp) in enumerate(remaining):
                rid = str(item.get("run_id", ""))
                corr_parent = _max_abs_corr(rid, parent_run_ids, corr_lookup)
                corr_selected = _max_abs_corr(rid, selected_ids, corr_lookup)
                penalty = 0.35 * corr_selected + (0.25 if exploration else 0.12) * corr_parent
...
def _query_plan_queries(query_plan: Dict[str, Any] | None, query_text: str = "") -> list[str]:
    queries: list[str] = []
    if query_plan:
    ...
    if query_text and query_text.strip():
        queries.insert(0, query_text.strip())
    deduped: list[str] = []
    for q in queries:
        if q not in deduped:
            deduped.append(q)
...
def _multi_query_passing_factors(
    passing: list[Dict[str, Any]],
    queries: list[str],
    *,
    semantic_k: int,
    ...
    parent_run_ids: list[str] | None = None,
    exploration: bool = False,
) -> tuple[list[tuple[str, Dict[str, Any], float | None]], str, Dict[str, Any]]:
    if not queries:
        return _query_guided_passing_factors(
            passing,
            "",
            semantic_k=semantic_k,
            anchor_count=anchor_count,
            max_factors=max_factors,
        )[0], "top", {"mode": "top", "queries": []}
    updated = _ensure_passing_factor_embeddings(passing)
    store = _load_embedding_store()
    vectors = store.get("vectors", {})
    model_name = _embedding_model()
    query_vecs = _embed_texts(queries, model=model_name) or []
    fused: dict[str, float] = {}
    sim_display: dict[str, float] = {}
    item_by_id = {str(item.get("run_id", "")): item for item in passing if item.get("run_id")}
    for q_idx, query in enumerate(queries):
        lexical = _lexical_similarity_scores(passing, query)
        ranked_ids: list[tuple[float, str, float]] = []
        q_vec = query_vecs[q_idx] if q_idx < len(query_vecs) else None
        for item in passing:
            rid = str(item.get("run_id", ""))
            sem = 0.0
            rec = vectors.get(rid, {})
            vec = rec.get("vector")
            if q_vec and rec.get("model") == model_name and isinstance(vec, list):
                sem = _dot_similarity(q_vec, vec)
            lex = lexical.get(rid, 0.0)
            score = sem * 0.72 + lex * 0.28
            ranked_ids.append((score, rid, sem))
        ranked_ids.sort(reverse=True)
        for rank, (score, rid, sem) in enumerate(ranked_ids, start=1):
            fused[rid] = fused.get(rid, 0.0) + 1.0 / (60.0 + rank)
            sim_display[rid] = max(sim_display.get(rid, 0.0), sem)
    total = max(len(passing), 1)
    ranked = []
    for idx, item in enumerate(passing):
        rid = str(item.get("run_id", ""))
        quality_prior = 1.0 - idx / total
        ranked.append((fused.get(rid, 0.0) + quality_prior * 0.01, item, sim_display.get(rid)))
    ranked.sort(key=lambda row: row[0], reverse=True)
    selected = _select_query_guided_rows(
        ranked,
        passing,
        primary_kind="multi_query",
        semantic_k=semantic_k,
        anchor_count=anchor_count,
        max_factors=max_factors,
    ...
    trace = {
        ...
        "selected_run_ids": [str(item.get("run_id", "")) for _, item, _ in selected],
        "top_sim": selected[0][2] if selected else None,
    }
    _trace_rag(
        "[Recall]",
        f"mode=multi_query passing={len(passing)} selected={len(selected)} queries={len(queries)} "
        f"anchors={anchor_count} semantic_k={semantic_k} top_sim={trace['top_sim'] if trace['top_sim'] is not None else '--'}",
    )
    return selected, "multi_query", trace
...
def _copy_submit_artifact(run_id: str, entry: Dict[str, Any]) -> Dict[str, Any]:
    """Copy passing factor parquet into autoalpha_v3/submit and return metadata fields."""
...
def add_factor(
    ...
    entry: Dict[str, Any] = {
        ...
        "full_2022_2024": result.get("full_2022_2024", {}),
        "period_metrics": result.get("period_metrics", []),
        "oos_comparison": result.get("oos_comparison", {}),
        ...
        "systematic_spec": result.get("systematic_spec", {}),
        "v2_source": result.get("v2_source", {}),
    ...
    if not entry.get("PassGates") and entry.get("formula"):
        cards = kb.setdefault("failure_cards", [])
        cards.append(_failure_card_from_entry(run_id, entry))
        del cards[:-240]
...
def sync_submit_artifacts() -> Dict[str, Any]:
    """Backfill autoalpha_v3/submit with all currently passing parquet factors."""
    kb = _load()
    factors = kb.setdefault("factors", {})
    copied = 0
    skipped = 0
...
def save_generation_experience(generation: int, markdown: str, payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    record = {
        ...
        "created_at": datetime.now().isoformat(),
        "path": str(note_path),
        "relative_path": str(note_path.relative_to(PROJECT_ROOT)),
...
def get_generation_guidance(
    ...
    for item in recent_failed:
        ...
        if float(corr.get("max_abs_corr", 0) or 0) >= 0.72:
            high_corr_rejects.append({
                "run_id": item.get("run_id", ""),
                "formula": item.get("formula", ""),
                "max_abs_corr": float(corr.get("max_abs_corr", 0) or 0),
                "closest_run_id": corr.get("closest_run_id", ""),
                "motif": formula_motif(item.get("formula", "")),
            })
    high_corr_rejects = sorted(high_corr_rejects, key=lambda item: item["max_abs_corr"], reverse=True)[:5]
    ...
    return {
        ...
        "high_correlation_rejections": high_corr_rejects,
        "strong_examples": [
            {
                "run_id": item.get("run_id", ""),
                "formula": item.get("formula", ""),
...
def compose_passing_factors_rag(
    ...
    queries = _query_plan_queries(query_plan, query_text)
    if len(queries) > 1:
        selected_rows, retrieval_mode, trace = _multi_query_passing_factors(
            passing,
            queries,
        ...
        selected_rows, retrieval_mode = _query_guided_passing_factors(
            passing,
            query_text=queries[0] if queries else query_text,
            semantic_k=min(max_factors, max(1, semantic_k)),
            anchor_count=min(max_factors, max(1, anchor_count)),
            max_factors=max_factors,
        )
        trace = {
            ...
            "queries": queries,
            "candidate_count": len(passing),
            "selected_run_ids": [str(item.get("run_id", "")) for _, item, _ in selected_rows],
        }
    global _LAST_RAG_TRACE
    _LAST_RAG_TRACE = trace
...
def _failure_card_from_entry(run_id: str, item: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "IC": float(item.get("IC", 0) or 0),
        "IR": float(item.get("IR", 0) or 0),
        "tvr": float(item.get("tvr", item.get("Turnover", 0)) or 0),
        "screen_fail_reason": item.get("screen_fail_reason", ""),
        "created_at": item.get("created_at", ""),
    }


def _failure_card_text(card: Dict[str, Any]) -> str:
    return (
        f"- run_id={card.get('run_id')} reason={card.get('reason')} "
        f"IC={float(card.get('IC', 0) or 0):.3f} IR={float(card.get('IR', 0) or 0):.2f} "
        f"TVR={float(card.get('tvr', 0) or 0):.0f} fail={card.get('screen_fail_reason', '')} "
        f"fingerprint={str(card.get('fingerprint', ''))[:120]} formula={str(card.get('formula', ''))[:160]}"
    )


def compose_failure_memory_context(
    query_plan: Dict[str, Any] | None = None,
    *,
    limit: int = 8,
    max_chars: int = 1200,
    ...
    kb = _load()
    cards = list(kb.get("failure_cards") or [])
    if not cards:
        rows = [
            (rid, info)
            for rid, info in kb.get("factors", {}).items()
            if info.get("formula") and not info.get("PassGates")
        ]
        rows.sort(key=lambda pair: pair[1].get("created_at", ""))
        cards = [_failure_card_from_entry(rid, info) for rid, info in rows[-60:]]
    if not cards:
        return ""
    query = " ".join(_query_plan_queries(query_plan, ""))
    if query:
        tokens = {t for t in re.split(r"[^a-zA-Z0-9_]+", query.lower()) if len(t) >= 3}
        cards.sort(
            key=lambda card: (
                sum(1 for token in tokens if token in json.dumps(card, ensure_ascii=False).lower()),
                card.get("created_at", ""),
            ),
            reverse=True,
        )
        ...
        cards = cards[-limit:]
    lines = ["## Structured Failure Memory", "Avoid repeating these failed mechanisms/skeletons:"]
    total = sum(len(line) + 1 for line in lines)
    for card in cards[:limit]:
        line = _failure_card_text(card)

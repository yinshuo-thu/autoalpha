...
def integrate_neural_network_positions(strategy_instance):
    ...
    # 训练模型
    if hasattr(strategy_instance, 'min5_data'):
        success = nn_generator.train(
            strategy_instance.features_data,
            strategy_instance.min5_data,
            epochs=30
        )
...
class NoFutureRiskManager:
    ...
    def __init__(self, max_position: float = 0.2, max_leverage: float = 1.5):
        ...
        # 调整风险参数以适应5分钟K线
        self.stop_loss_multiplier = 2.0  # 降低止损倍数（5分钟波动较小）
        self.take_profit_multiplier = 2.5  # 降低止盈倍数
        self.max_daily_trades = 30  # 增加每日交易次数限制（5分钟机会更多）
        self.min_holding_period = 3  # 最小持有周期（12个5分钟=1小时）
...
class NoFutureSignalGenerator:
    def __init__(self, model_trainer):
        ...
        self.signal_delay = 1  # 确保至少2个周期的延迟
...
class NoFutureBacktestEngine:
   def __init__(self, initial_capital: float = 1000000, commission_rate: float = 0.0002):
        ...
        # 修改为5分钟K线的周期参数
        self.periods_per_day = 96  # 8小时交易时间，每5分钟一根K线
...
class NoFutureModelTrainer:
    ...
    @monitor_memory
    def prepare_training_data_parallel(self, features_dict: Dict, min5_data_dict: Dict,
        ...
        args_list = []
        for symbol in features_dict:
            if symbol not in min5_data_dict:
                continue
                
            features_df, feature_cols = features_dict[symbol]
            min5_data = min5_data_dict[symbol]
            
            args = (symbol, features_df, feature_cols, min5_data, 
...
def process_single_symbol_data_no_future(args):
    """
    完全无未来函数的单品种数据处理函数 - 5分钟K线版本
      
    Args:
        args: (symbol, data_path, cache_dir, sample_rate, return_1m_data)
      
    Returns:
        (symbol, min5_data, min1_data, from_cache)
    """
    # 解包参数，支持旧版本兼容
    if len(args) == 4:
    ...
    try:
        ...
        cache_params = {
            ...
            'version': 'no_future_5min_v1',  # 改为5分钟版本
            'return_1m': return_1m_data
        }
        cached_data = cache.load_data(symbol, 'min5_data_multiscale', cache_params)
          
        if cached_data is not None:
            logger.info(f"Loaded cached data for {symbol}")
            min5_data = cached_data.get('min5_data')
            min1_data = cached_data.get('min1_data', None)
            return symbol, min5_data, min1_data, True
        ...
        logger.info(f"Processing {symbol} with 5-minute bars...")
        ...
        # 生成5分钟K线（改为5T）
        min5_data = df_1m.resample('5T').agg({
            'open': 'first',
            'high': 'max',
            'low': 'min',
            'close': 'last',
        ...
        min5_data = min5_data.dropna()
          
        # 采样处理
        if sample_rate < 1.0 and len(min5_data) > 100:
            original_len = len(min5_data)
              
            if return_1m_data:
                # 如果需要1分钟数据，使用尾部连续采样
                sample_size = int(len(min5_data) * sample_rate)
                min5_data = min5_data.iloc[-sample_size:]
                  
                # 对应的1分钟数据也要截取
                start_time = min5_data.index[0]
                df_1m = df_1m[df_1m.index >= start_time]
            else:
                # 不需要1分钟数据时，可以随机采样
                sample_size = int(len(min5_data) * sample_rate)
                sample_indices = np.sort(np.random.choice(len(min5_data), sample_size, replace=False))
                min5_data = min5_data.iloc[sample_indices]
                df_1m = None  # 不保留1分钟数据
              
            logger.info(f"  Sampled {sample_rate*100:.0f}% of data: {original_len} -> {len(min5_data)} bars")
          
        # 清理数值数据
        min5_data = clean_numeric_data_no_future(min5_data)
          
        logger.info(f"  Final 5min bars: {len(min5_data)}, 1min data: {'Yes' if df_1m is not None else 'No'}")
          
        # 保存到缓存
        if len(min5_data) > 500:
            cache_data = {
                'min5_data': min5_data,
                'min1_data': df_1m if return_1m_data else None
            }
            cache.save_data(symbol, 'min5_data_multiscale', cache_data, cache_params)
            return symbol, min5_data, df_1m if return_1m_data else None, False
        else:
            logger.warning(f"  Insufficient data after processing: {len(min5_data)} bars")
            return symbol, None, None, False
...
def calculate_safe_technical_indicators_5min(df: pd.DataFrame) -> pd.DataFrame:
    """
    5分钟K线的技术指标计算 - 调整周期参数以适应5分钟尺度
    """
    logger.info(f"计算5分钟K线技术指标，数据形状: {df.shape}")
     
    # 基础价格特征
    df['returns'] = df['close'].pct_change()
    df['log_returns'] = np.log(df['close'] / df['close'].shift(1))
     
    # 价格动量 - 调整周期（5分钟K线需要更多的bar数来代表相同时间跨度）
    # 原30分钟的5根 = 150分钟，5分钟需要30根
    for period in [30, 60, 120]:  # 原来是5, 10, 20
        df[f'momentum_{period}'] = df['close'] / df['close'].shift(period) - 1
        df[f'roc_{period}'] = (df['close'] - df['close'].shift(period)) / df['close'].shift(period)
     
    # RSI计算 - 调整周期
    for period in [3, 6, 12]:  # 原来是6, 12, 24
        delta = df['close'].diff()
        gain = delta.where(delta > 0, 0)
        loss = -delta.where(delta < 0, 0)
    ...
    # MACD - 调整周期
    ema_72 = df['close'].ewm(span=72, adjust=False, min_periods=72).mean()  # 原12*6
    ema_156 = df['close'].ewm(span=156, adjust=False, min_periods=156).mean()  # 原26*6
    df['macd'] = ema_72 - ema_156
    df['macd_signal'] = df['macd'].ewm(span=54, adjust=False, min_periods=54).mean()  # 原9*6
    df['macd_diff'] = df['macd'] - df['macd_signal']
     
    # 布林带 - 调整周期
    for period in [60, 120]:  # 原10, 20
        min_periods = max(period // 2, 20)
        sma = df['close'].rolling(window=period, min_periods=min_periods, center=False).mean()
        std = df['close'].rolling(window=period, min_periods=min_periods, center=False).std()
        df[f'bb_upper_{period}'] = sma + 2 * std
        df[f'bb_lower_{period}'] = sma - 2 * std
    ...
    # 成交量相关指标 - 调整周期
    df['volume_ma_120'] = df['volume'].rolling(window=120, min_periods=60, center=False).mean()
    df['volume_ratio'] = df['volume'] / df['volume_ma_120']
    ...
    df['atr_84'] = tr.rolling(window=84, min_periods=42, center=False).mean()  # 原14*6
...
def process_features_for_symbol_no_future(args):
    ...
    """
    if len(args) == 3:
        symbol, min5_data, cache_dir = args
        min1_data = None
    else:
        symbol, min5_data, cache_dir, min1_data = args
...
def prepare_training_data_for_symbol_no_future(args):
    ...
    """
    symbol, features_df, feature_cols, min5_data, look_ahead, min_samples, is_training = args
    ...
    try:
        ...
        # 数据对齐
        common_index = features_df.index.intersection(min5_data.index)
        ...
        aligned_prices = min5_data.loc[common_index]
...
class NoFutureDataLoader:
   ...
   def load_multiple_symbols_parallel(self, symbols: List[str], 
        ...
        logger.info(f"Successfully loaded {len(min5_data)} symbols")
   ...
   @monitor_memory
   def generate_features_parallel(self, min5_data_dict: Dict[str, pd.DataFrame],
                                min1_data_dict: Dict[str, pd.DataFrame] = None):
        """
        并行生成特征 - 支持多时间尺度
          
        Args:
            min5_data_dict: 30分钟数据字典
            min1_data_dict: 1分钟数据字典（可选）
          
        Returns:
            特征字典
        """
        logger.info(f"Generating features for {len(min5_data_dict)} symbols...")
        ...
        # 准备参数列表
        args_list = []
        for symbol, data_30m in min5_data_dict.items():
        ...
        with ProcessPoolExecutor(max_workers=10) as executor:
            ...
            with tqdm(total=len(min5_data_dict), desc="Generating features") as pbar:
                for future in as_completed(future_to_symbol):
                    symbol = future_to_symbol[future]
                    try:
...
class NoFutureCommodityStrategy:
    def __init__(self, data_path: str, info_path: str, use_gpu: bool = True, 
        ...
        # 5分钟K线特定配置
        self.bar_minutes = 5
        self.periods_per_hour = 12  # 60分钟 / 5分钟
        self.periods_per_day = 96   # 8小时交易时间
        self.prediction_horizon = 3  # 预测3个bar = 15分钟
    ...
    def load_and_process_data(self, symbols: List[str] = None, 
        ...
        logger.info(f"Successfully loaded {len(self.min5_data)} symbols")
        
        if not self.min5_data:
        ...
        feature_args_list = []
        for symbol, data_5m in self.min5_data.items():
        ...
        with ProcessPoolExecutor(max_workers=self.data_loader.n_processes) as executor:
            ...
            with tqdm(total=len(self.min5_data), desc="Generating features") as pbar:
                for future in as_completed(future_to_symbol):
                    symbol = future_to_symbol[future]
                    try:
                        symbol, features_df, feature_cols, from_cache = future.result()
        ...
        logger.info(f"  - Symbols loaded: {len(self.min5_data)}")
        logger.info(f"  - Features generated: {len(self.features_data)}")
        
        total_5m_bars = sum(len(df) for df in self.min5_data.values())
        ...
        # 时间覆盖分析
        if self.min5_data:
            sample_symbol = list(self.min5_data.keys())[0]
            sample_data = self.min5_data[sample_symbol]
    ...
    def prepare_and_train_models(self, look_ahead: int = 3, min_samples: int = 300) -> bool:
        ...
        # 1. 检查数据
        if not self.min5_data or not self.features_data:
        ...
        if hasattr(self.model_trainer, 'prepare_training_data_parallel'):
            ...
            for symbol in self.features_data:
                if symbol not in self.min5_data:
                    continue
                    
                features_df, feature_cols = self.features_data[symbol]
                min5_data = self.min5_data[symbol]
    ...
    def run_full_strategy(self, max_symbols: int = 10,
        ...
        if enable_validation:
            ...
            for symbol in self.min5_data:
                df = self.min5_data[symbol]
        ...
        if enable_validation:
            ...
            for symbol in self.signals:
                if symbol not in self.features_data or symbol not in self.min5_data:
                    continue
 
                features_df, _ = self.features_data[symbol]
                signals = self.signals[symbol]
                prices = self.min5_data[symbol]
        ...
        backtest_results = self.backtest_engine.run_backtest(
            self.signals, self.min5_data, start_date, end_date
        )
    ...
    def _generate_ml_positions(self):
        ...
        positions_dict = self.improved_strategy.generate_positions(self.min5_data)  # 改为min5_data
        ...
        for symbol, positions in positions_dict.items():
            ...
            # 验证
            if symbol in self.features_data and symbol in self.min5_data:  # 改为min5_data
                features_df, _ = self.features_data[symbol]
                try:
                    validate_data_alignment(
                        features_df,
                        positions,
                        self.min5_data[symbol]  # 改为min5_data
                    )
                except AssertionError as e:
...
def integrate_intelligent_position_manager(strategy_instance):
    ...
    # 如果有历史数据，先训练初始模型
    if hasattr(strategy_instance, 'min5_data') and strategy_instance.min5_data:
        for symbol, data in strategy_instance.min5_data.items():

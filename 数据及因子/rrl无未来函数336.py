...
class RRLTrainer:
    ...
    def train_batch(self, features: np.ndarray, prices: np.ndarray,
        ...
        return loss.item()
    def returns_based_loss(self, returns: torch.Tensor, positions: torch.Tensor) -> torch.Tensor:
            """
            基于收益率的损失函数（替代夏普比率）
            使用多个目标的加权组合，更稳定且直接
...
def create_trading_target_variable_no_future(df: pd.DataFrame, horizon: int = 5, 
                                            is_training: bool = True) -> pd.Series:
    """
    修正版本：完全不使用未来数据的目标变量生成
    即使在训练时也使用历史模式识别而非未来收益
    """
    close_prices = df['close'].copy()
    target = pd.Series(index=df.index, dtype=float)
    target[:] = 0
     
    # 不再区分训练和预测模式，都不使用未来数据
    logger.info("Target creation without any future information")
     
    # 使用历史模式识别方法生成标签
    for i in range(100, len(df)):  # 需要足够的历史数据
        # 只使用i之前的数据
        hist_prices = close_prices.iloc[max(0, i-100):i]
        ...
        # 1. 趋势强度
        trend_strength = (hist_prices.iloc[-1] / hist_prices.iloc[-20] - 1) if len(hist_prices) >= 20 else 0
         
        # 2. 动量
        short_momentum = (hist_prices.iloc[-1] / hist_prices.iloc[-5] - 1) if len(hist_prices) >= 5 else 0
        medium_momentum = (hist_prices.iloc[-1] / hist_prices.iloc[-10] - 1) if len(hist_prices) >= 10 else 0
         
        # 3. 波动率
        volatility = hist_returns.std() if len(hist_returns) > 10 else 0.02
        ...
        rs = gain.iloc[-1] / (loss.iloc[-1] + 1e-10) if len(gain) > 0 else 1
        rsi = 100 - (100 / (1 + rs))
         
        # 5. 支撑/阻力位
        recent_high = hist_prices.iloc[-20:].max() if len(hist_prices) >= 20 else hist_prices.iloc[-1]
        recent_low = hist_prices.iloc[-20:].min() if len(hist_prices) >= 20 else hist_prices.iloc[-1]
        ...
        # 趋势跟随信号
        if trend_strength > 0.02 and short_momentum > 0 and rsi < 70:
            signal_strength += min(trend_strength * 10, 0.5)
        elif trend_strength < -0.02 and short_momentum < 0 and rsi > 30:
            signal_strength -= min(abs(trend_strength) * 10, 0.5)
        ...
        # 波动率调整
        vol_adj = min(1.0, 0.02 / (volatility + 1e-10))
        signal_strength *= vol_adj
         
        # 交易成本考虑
        cost_threshold = 0.0006
...
class NoFutureSignalGenerator:
    def __init__(self, model_trainer):
        ...
        self.signal_delay = 2  # 确保至少2个周期的延迟
    ...
    def generate_signals_from_features(self, features_df, feature_cols, df, symbol, 
        ...
        try:
            ...
            # 添加额外验证
            if self.leakage_checker:
                # 验证信号不包含未来信息
                for i in range(len(final_signals) - safety_delay):
                    if i > 0 and abs(final_signals.iloc[i]) > 0:
                        # 确保信号基于过去的数据
                        assert i >= safety_delay, f"Signal at index {i} may use future data"
...
def prepare_training_data_for_symbol_no_future(args):
    ...
    try:
        ...
        if not common_index.is_monotonic_increasing:
            logger.warning(f"{symbol}: Time series is not monotonic increasing, sorting...")
        ...
        # ===== 修改：使用不含未来信息的目标变量生成 =====
        if is_training:
            # 训练模式：基于历史模式生成标签（不使用未来收益）
            target = pd.Series(index=common_index, dtype=float)
            target[:] = 0
             
            for i in range(100, len(aligned_prices)):  # 需要足够的历史
                if i >= len(aligned_prices) - look_ahead * 2:
                    # 接近数据末尾，停止生成标签
                    target.iloc[i] = 0
                    continue
                 
                # 只使用i之前的数据计算特征
                hist_prices = aligned_prices['close'].iloc[max(0, i-100):i]
                ...
                if len(hist_prices) >= 20:
                    trend = (hist_prices.iloc[-1] / hist_prices.iloc[-20] - 1)
                    momentum = (hist_prices.iloc[-1] / hist_prices.iloc[-5] - 1) if len(hist_prices) >= 5 else 0
                else:
                    trend = 0
                    momentum = 0
                ...
                # 趋势跟随逻辑
                if abs(trend) > 0.02:
                    if trend > 0 and momentum > 0 and rsi < 70:
                        signal_strength += min(abs(trend) * 5, 0.5)
                    elif trend < 0 and momentum < 0 and rsi > 30:
                        signal_strength -= min(abs(trend) * 5, 0.5)
                ...
                # 交易成本过滤
                if abs(signal_strength) < 0.001:
                    signal_strength = 0
                 
                target.iloc[i] = np.clip(signal_strength, -1.0, 1.0)
             
            # 清理末尾数据
            target.iloc[-look_ahead*2:] = 0
        ...
        if len(inf_cols) > 0:
            logger.warning(f"{symbol}: Found inf values in columns: {inf_cols.tolist()[:5]}")
            X[inf_cols] = X[inf_cols].replace([np.inf, -np.inf], np.nan)
         
        # 检查NaN
        nan_ratio = X.isna().sum().sum() / (X.shape[0] * X.shape[1])
        if nan_ratio > 0.1:
            logger.warning(f"{symbol}: High NaN ratio: {nan_ratio:.2%}")
        ...
        if is_training:
            ...
            if non_zero_ratio < 0.01:  # 少于1%的非零标签
            ...
            logger.info(f"{symbol}: Label distribution - Positive: {positive_ratio:.1%}, "
                       f"Negative: {negative_ratio:.1%}, Zero: {(y == 0).mean():.1%}")
        ...
        if valid_rows.sum() > min_samples:
            ...
            # 验证没有未来信息泄露
            if is_training and len(final_y) > 100:
                # 简单检查：标签不应该与未来特征高度相关
                for col in final_X.columns[:10]:  # 检查前10个特征
                    future_feature = final_X[col].shift(-1)
                    correlation = final_y.corr(future_feature)
                    if abs(correlation) > 0.8:
                        logger.warning(f"{symbol}: Suspicious correlation with future {col}: {correlation:.3f}")
...
class NoFutureCommodityStrategy:
    def __init__(self, data_path: str, info_path: str, use_gpu: bool = True, 
                 ...
                 enable_validation: bool = True,  # 直接作为参数
                 strict_validation: bool = False):  # 直接作为参数
    ...
    def run_full_strategy(self, max_symbols: int = 10,
        ...
        if enable_validation:
            ...
            for symbol in self.min30_data:
                df = self.min30_data[symbol]
        ...
        if enable_validation:
            ...
            for symbol in self.signals:
                if symbol not in self.features_data or symbol not in self.min30_data:
                    continue
 
                features_df, _ = self.features_data[symbol]
                signals = self.signals[symbol]
                prices = self.min30_data[symbol]
    ...
    def _generate_ml_positions(self):
        ...
        for symbol, positions in positions_dict.items():
            ...
            # ===== 在这里验证 =====
            if symbol in self.features_data and symbol in self.min30_data:
                features_df, _ = self.features_data[symbol]
                try:
                    validate_data_alignment(
                        features_df,
                        positions,
                        self.min30_data[symbol]
                    )
                except AssertionError as e:
                    logger.warning(f"{symbol}: 信号验证失败 - {e}")
                    # 如果验证失败，清零该品种的信号

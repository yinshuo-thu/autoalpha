...
# ================== RRL神经网络模型 ==================
class ImprovedRRLModel(nn.Module):
    """改进的RRL模型，基于领导的架构建议"""
    
    def __init__(self, feature_num: int, hidden_sizes: List[int] = [128, 64, 32]):
        super(ImprovedRRLModel, self).__init__()
        self.feature_num = feature_num
        
        # Factor Interaction Branch (使用Tanh处理因子交互)
        layers = []
        input_size = feature_num
        for hidden_size in hidden_sizes:
            layers.append(nn.Linear(input_size, hidden_size))
            layers.append(nn.Tanh())  # Tanh适合处理因子的双向关系
            layers.append(nn.Dropout(0.1))  # 防止过拟合
            input_size = hidden_size
        self.factor_branch = nn.Sequential(*layers[:-1])  # 去掉最后的dropout
        
        # Position-Signal Interaction Branch (使用ReLU处理仓位信号交互)
        self.position_branch = nn.Sequential(
            nn.Linear(hidden_sizes[-1] + 1, hidden_sizes[-1]),  # +1 for position
            nn.ReLU(),  # ReLU适合处理正向激活
            nn.Dropout(0.1),
            nn.Linear(hidden_sizes[-1], 16),
            nn.ReLU(),
            nn.Linear(16, 1),
            nn.Sigmoid()  # 输出0-1之间的调整比例
        )
        
        # Final position adjustment branch
        self.position_head = nn.Sequential(
            nn.Linear(hidden_sizes[-1], 1),
            nn.Tanh()  # 输出-1到1之间的目标仓位
        )
    ...
    def forward(self, features: torch.Tensor, pos: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            features: 特征张量 [batch_size, feature_num]
            pos: 当前仓位 [batch_size, 1]
    ...
    def generate_signals_from_features(self, features_df, feature_cols, df, symbol, 
        ...
        try:
            if len(features_df) < 100:
                ...
                return pd.Series(0, index=features_df.index, dtype=float)  # 返回零序列而不是空序列
            ...
            # 获取预测（预期收益）
            try:
                # 检查模型训练器是否有 predict_ensemble 方法
            ...
            return pd.Series(0, index=features_df.index, dtype=float)  # 确保返回有效序列
...
if __name__ == "__main__":
    ...
    try:
        ...
        strategy = NoFutureCommodityStrategy(
            ...
            use_gpu=True,
            use_improved_components=True  # 使用改进组件
        )

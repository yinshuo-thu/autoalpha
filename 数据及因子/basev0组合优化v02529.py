...
class NoFutureRiskManager:
    ...
    def calculate_position_size_optimized(self, signal: float, symbol: str, 
        ...
        """
        # 获取上一期仓位
        w_pre = self.position_optimizer.get_previous_position(symbol)
        ...
        if volatility is not None and volatility > 0:
            ...
            adjusted_signal = signal * vol_adj
        else:
            adjusted_signal = signal
            
        # 使用分段线性函数计算最优仓位
        optimal_position = self.position_optimizer.calculate_optimal_position(
            adjusted_signal, w_pre
        )
        
        # 应用风险限制
        optimal_position = np.clip(optimal_position, -self.max_position, self.max_position)
        
        # 更新仓位历史
        self.position_optimizer.update_position_history(symbol, optimal_position)
...
class NoFutureSignalGenerator:
    ...
    def __init__(self, model_trainer):
        ...
        self.signal_scale_factor = 0.1  # 信号缩放因子，控制信号强度

...
import wandb
# Add this class definition after your imports and before the NoFutureModelTrainer class

class OptunaWandbOptimizer:
    """W&B-enhanced Optuna hyperparameter optimizer"""
    
    def __init__(self, project_name: str = None, entity: str = None, 
                 use_gpu: bool = True, n_trials: int = 50):
    ...
    def prepare_validation_split(self, X_train, y_train, val_ratio=0.2):
        ...
        logger.info(f"Optuna data split: Train {len(self.X_train)}, Val {len(self.X_val)}")
    
    def objective_xgboost(self, trial):
        """XGBoost objective with W&B logging"""
        params = {
            'objective': 'reg:squarederror',
            'max_depth': trial.suggest_int('max_depth', 3, 10),
            'learning_rate': trial.suggest_float('learning_rate', 0.01, 0.3, log=True),
        ...
        if self.wandb_run:
            wandb.log({
                "xgboost/trial": trial.number,
                "xgboost/rmse": rmse,
                "xgboost/correlation": correlation,
                "xgboost/direction_acc": direction_acc,
                **{f"xgboost/{k}": v for k, v in params.items() if isinstance(v, (int, float))}
            })
        
        return direction_acc
    
    def objective_lightgbm(self, trial):
        """LightGBM objective with W&B logging"""
        params = {
            'objective': 'regression',
            'metric': 'rmse',
            'boosting_type': 'gbdt',
        ...
        if self.wandb_run:
            wandb.log({
                "lightgbm/trial": trial.number,
                "lightgbm/rmse": rmse,
                "lightgbm/correlation": correlation,
                "lightgbm/direction_acc": direction_acc,
                **{f"lightgbm/{k}": v for k, v in params.items() if isinstance(v, (int, float))}
            })
        
        trial.report(direction_acc, model.best_iteration)
        if trial.should_prune():
            raise optuna.TrialPruned()
    ...
    def objective_catboost(self, trial):
        """CatBoost objective with W&B logging"""
        params = {
            'iterations': trial.suggest_int('iterations', 100, 500, step=50),
            'learning_rate': trial.suggest_float('learning_rate', 0.01, 0.3, log=True),
            'depth': trial.suggest_int('depth', 3, 10),
        ...
        if self.wandb_run:
            wandb.log({
                "catboost/trial": trial.number,
                "catboost/rmse": rmse,
                "catboost/correlation": correlation,
                "catboost/direction_acc": direction_acc,
                **{f"catboost/{k}": v for k, v in params.items() if isinstance(v, (int, float))}
            })
        
        trial.report(direction_acc, model.get_best_iteration())
        if trial.should_prune():
            raise optuna.TrialPruned()
        
        return direction_acc
    
    def optimize(self, X_train, y_train, model_types=None):
        """Run optimization with W&B tracking"""
        if model_types is None:
            model_types = ['xgboost', 'lightgbm', 'catboost']
        
        # Initialize W&B run if project specified
        ...
        for model_type in model_types:
            if model_type == 'xgboost':
                ...
                study.optimize(self.objective_xgboost, n_trials=self.n_trials, n_jobs=1)
                ...
                study.optimize(self.objective_lightgbm, n_trials=self.n_trials, n_jobs=1)
                ...
                study.optimize(self.objective_catboost, n_trials=self.n_trials, n_jobs=1)
    ...
    def save_best_params(self, filepath):
        ...
        with open(filepath, 'w') as f:
            json.dump(self.best_params, f, indent=2)
        logger.info(f"Best params saved to: {filepath}")
    
    def load_best_params(self, filepath):
        """Load best parameters"""
        with open(filepath, 'r') as f:
            self.best_params = json.load(f)
        logger.info(f"Loaded best params from {filepath}")
        return self.best_params
# Modify the NoFutureModelTrainer class to include W&B tracking
class NoFutureModelTrainer:
    """完全无未来函数的模型训练器 - Enhanced with W&B tracking"""
    
    def __init__(self, use_gpu: bool = True, use_optuna: bool = False, 
                 optuna_trials: int = 30, wandb_project: str = None, 
    ...
    def optimize_hyperparameters(self, X_train, y_train):
        """使用Optuna优化超参数 - with W&B tracking"""
        if not self.use_optuna:
            return
        
        logger.info("开始Optuna超参数优化 (with W&B tracking)...")
        
        # Use W&B-enhanced optimizer if project specified
        if self.wandb_project:
            self.optimizer = OptunaWandbOptimizer(
        ...
        # Run optimization
        self.optimized_params = self.optimizer.optimize(
            X_train, y_train, 
            model_types=['xgboost', 'lightgbm', 'catboost']
        )
        ...
        # Finish W&B run if using W&B optimizer
        if hasattr(self.optimizer, 'finish'):
            self.optimizer.finish()
        
        # Save optimization results
        params_file = os.path.expanduser('~/autodl-tmp/best_params_with_wandb.json')
...
if __name__ == "__main__":
    print("Starting Complete No-Future-Function strategy with W&B-tracked Optuna Optimization...")
    
    # Initialize W&B (one-time login required: run 'wandb login' in terminal)
    wandb.login(key="eee89d51af35e33f64fe7825eae6d4ae5cd82260")  # Optional: provide API key directly
    
    # Create strategy instance with W&B tracking enabled
    ...
    # Modify the model trainer to include W&B tracking
    strategy.model_trainer.wandb_project = "commodity-futures-optimization"
    strategy.model_trainer.wandb_entity = "your-wandb-username"  # Replace with your W&B username
    ...
    print("\n" + "="*60)
    print("策略执行结果检查 (with W&B tracking)：")
    ...
    if results is None:
        ...
        if wandb.run is not None:
            wandb.log({
                "final/total_return": results.get('total_return', 0),
                "final/annual_return": results.get('annual_return', 0),
                "final/sharpe_ratio": results.get('sharpe_ratio', 0),
                "final/max_drawdown": results.get('max_drawdown', 0),
                "final/total_trades": results.get('total_trades', 0),
                "final/win_rate": results.get('win_rate', 0)
            })
            
            # Create summary table
            summary_table = wandb.Table(columns=["Metric", "Value"])
            summary_table.add_data("Total Return", f"{results.get('total_return', 0):.2%}")
            summary_table.add_data("Annual Return", f"{results.get('annual_return', 0):.2%}")
            summary_table.add_data("Sharpe Ratio", f"{results.get('sharpe_ratio', 0):.2f}")
            summary_table.add_data("Max Drawdown", f"{results.get('max_drawdown', 0):.2%}")
            summary_table.add_data("Total Trades", results.get('total_trades', 0))
            summary_table.add_data("Win Rate", f"{results.get('win_rate', 0):.2%}")
            
            wandb.log({"final_summary": summary_table})
            
            print("\n✅ Results have been logged to W&B!")
        ...
        if results.get('total_trades', 0) > 0:
            ...
            print("\n📊 W&B tracking enabled - Check your W&B dashboard for detailed hyperparameter performance")
        else:
            print("\n策略执行成功但没有产生交易")
            print("可能需要调整信号生成参数")
    else:

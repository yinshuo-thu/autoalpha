...
Inspired by karpathy/autoresearch's ratchet pattern:
  ...
  - Loop logs to autoalpha_v3/loop.log for frontend streaming

Usage:
    python autoalpha_v3/loop.py --rounds 5 --ideas 3 --days 0
"""
...
from autoalpha_v3.pipeline import run
from autoalpha_v3.idea_cache import get_default_cache
from autoalpha_v3.inspiration_fetcher import start_background_fetcher
...
MODEL_LAB_LATEST_SUMMARY = Path(__file__).resolve().parent / "model_lab" / "latest_summary.json"
...
def _latest_model_lab_factor_count() -> int:
    """Return the factor count used by the latest model-lab snapshot."""
    if not MODEL_LAB_LATEST_SUMMARY.is_file():
        return 0
    try:
        import json

        payload = json.loads(MODEL_LAB_LATEST_SUMMARY.read_text(encoding="utf-8"))
        return int(payload.get("selected_factor_count", 0) or 0)
    except Exception:
        return 0
...
def mining_loop(
    ...
    source_config = str(cfg.get("AUTOALPHA_INSPIRATION_SOURCES", "paper,llm,manual"))
    source_set = {
        source.strip().lower()
    ...
    try:
        ...
        round_pause_sec = 1.0
    try:
        min_full_eval_days = int(cfg.get("AUTOALPHA_MIN_FULL_EVAL_DAYS", "450") or 700)
    except (TypeError, ValueError):
        min_full_eval_days = 450
    try:
    ...
    # Start background inspiration fetcher (Paper + LLM brainstorm + factor prompt docs)
    try:
        start_background_fetcher(
            interval_seconds=inspiration_interval,
            llm_ideas=1 if "llm" in source_set else 0,
            paper_results=12,
            manual_files=12 if "manual" in source_set else 0,
    ...
    initial_full_valid_count = len(kb.list_valid_factors(min_eval_days=min_full_eval_days))
    last_model_lab_valid = min(_latest_model_lab_factor_count(), initial_full_valid_count)
    loop_started_at = time.time()
    try:
        model_lab_min_factors = int(cfg.get("AUTOALPHA_MODEL_LAB_MIN_FACTORS", "10") or 10)
    except (TypeError, ValueError):
        model_lab_min_factors = 10
    try:
        model_lab_min_runtime_sec = int(cfg.get("AUTOALPHA_MODEL_LAB_MIN_RUNTIME_SEC", "3600") or 3600)
    except (TypeError, ValueError):
        model_lab_min_runtime_sec = 3600
    _log(
        "[model-lab] Auto trigger anchor: "
        f"latest_snapshot_factors={last_model_lab_valid} "
        f"current_full_window_valid={initial_full_valid_count}"
    )
    ...
    while True:
        ...
        try:
            results = run(
                ...
                idea_index_offset=(round_i - 1) * n_ideas_per_round,
            )
        ...
        model_lab_elapsed = time.time() - loop_started_at
        if (
            model_lab_interval > 0
            and full_valid_count >= model_lab_min_factors
            and model_lab_elapsed >= model_lab_min_runtime_sec
            and full_valid_count - last_model_lab_valid >= model_lab_interval
        ):
            _run_model_lab_snapshot(cfg, n_ideas_per_round, eval_days_count, target_valid_count, min_full_eval_days)
            last_model_lab_valid = full_valid_count
        ...
        try:
            idea_cache.start_fill(
                n=cache_prefill,
                parents=next_parents if next_parents else None,
                idea_index_offset=round_i * n_ideas_per_round,
            )

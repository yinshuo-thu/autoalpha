...
LAUNCH_LOG_DIR="${HOME}/Library/Logs/autoalpha_v3"
BACKEND_LABEL="com.autoalpha.v3.backend"
FRONTEND_LABEL="com.autoalpha.v3.frontend"
BACKEND_PLIST="${LAUNCH_AGENT_DIR}/${BACKEND_LABEL}.plist"
FRONTEND_PLIST="${LAUNCH_AGENT_DIR}/${FRONTEND_LABEL}.plist"
LEGACY_V2_BACKEND_LABEL="com.autoalpha.v2.backend"
LEGACY_V2_FRONTEND_LABEL="com.autoalpha.v2.frontend"
LEGACY_V2_BACKEND_PLIST="${LAUNCH_AGENT_DIR}/${LEGACY_V2_BACKEND_LABEL}.plist"
LEGACY_V2_FRONTEND_PLIST="${LAUNCH_AGENT_DIR}/${LEGACY_V2_FRONTEND_LABEL}.plist"
LEGACY_BACKEND_LABEL="com.scientech.autoalpha.backend"
LEGACY_FRONTEND_LABEL="com.scientech.autoalpha.frontend"
LEGACY_TUNNEL_LABEL="com.scientech.autoalpha.tunnel"
LEGACY_BACKEND_PLIST="${LAUNCH_AGENT_DIR}/${LEGACY_BACKEND_LABEL}.plist"
LEGACY_FRONTEND_PLIST="${LAUNCH_AGENT_DIR}/${LEGACY_FRONTEND_LABEL}.plist"
LEGACY_TUNNEL_PLIST="${LAUNCH_AGENT_DIR}/${LEGACY_TUNNEL_LABEL}.plist"
LAUNCH_DOMAIN="gui/$(id -u)"

export PATH="/opt/homebrew/bin:/usr/local/bin:${PATH}"
export AUTOALPHA_APP_BASE="${AUTOALPHA_APP_BASE:-/v3/}"
...
Starts the AutoAlpha v3 backend and frontend.
...
if [[ "$REUSE" == true ]]; then
  ...
  if [[ "$(uname -s)" == "Darwin" && -x "$(command -v launchctl)" ]]; then
    ...
    launch_agent_stop "$LEGACY_V2_BACKEND_LABEL" "$LEGACY_V2_BACKEND_PLIST"
    launch_agent_stop "$LEGACY_V2_FRONTEND_LABEL" "$LEGACY_V2_FRONTEND_PLIST"
    launch_agent_stop "$LEGACY_BACKEND_LABEL" "$LEGACY_BACKEND_PLIST"
    launch_agent_stop "$LEGACY_FRONTEND_LABEL" "$LEGACY_FRONTEND_PLIST"
    launch_agent_stop "$LEGACY_TUNNEL_LABEL" "$LEGACY_TUNNEL_PLIST"
  fi
...
if [[ "${backend_ok:-false}" != true ]]; then
  ...
  if [[ "$USE_LAUNCHCTL" == true ]]; then
    : > "$BACKEND_LOG"
    backend_cmd="cd \"${ROOT_DIR}\" && AUTOALPHA_APP_BASE=\"${AUTOALPHA_APP_BASE}\" exec \"${PYTHON}\" \"${ROOT_DIR}/server.py\" >> \"${BACKEND_LOG}\" 2>&1"
    ...
    cd "$ROOT_DIR"
    nohup env AUTOALPHA_APP_BASE="$AUTOALPHA_APP_BASE" "$PYTHON" server.py > "$BACKEND_LOG" 2>&1 < /dev/null &
    BACKEND_PID=$!
...
if [[ "${frontend_ok:-false}" != true ]]; then
  ...
  if [[ "$USE_LAUNCHCTL" == true ]]; then
    : > "$FRONTEND_LOG"
    frontend_cmd="cd \"${FRONTEND_DIR}\" && AUTOALPHA_APP_BASE=\"${AUTOALPHA_APP_BASE}\" exec \"${NPM}\" run dev -- --host \"${FRONTEND_HOST}\" --port \"${FRONTEND_PORT}\" >> \"${FRONTEND_LOG}\" 2>&1"
    ...
    cd "$FRONTEND_DIR"
    nohup env AUTOALPHA_APP_BASE="$AUTOALPHA_APP_BASE" "$NPM" run dev -- --host "$FRONTEND_HOST" --port "$FRONTEND_PORT" > "$FRONTEND_LOG" 2>&1 < /dev/null &
    FRONTEND_PID=$!
...
===========================================
AutoAlpha v3 services are running
Backend : ${BACKEND_URL}

#!/usr/bin/env python3
"""Read-only AutoAlpha v3 display server.
...
def add_display_cache_headers(response):
    ...
    if path.startswith("/v3/assets/"):
        response.headers["Cache-Control"] = "public, max-age=31536000, immutable"
    elif path == "/v3/st-logo.png" or path == "/st-logo.png":
        response.headers["Cache-Control"] = "public, max-age=86400"
    elif path in {"/v3", "/v3/"} or path.startswith("/api/"):

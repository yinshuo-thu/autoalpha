...
OPERATORS = {
    ...
    'cs_scale':   {'category': 'cross_section', 'params': 'x', 'desc': 'Alias for scale(x)',
                   'example': 'cs_scale(volume)', 'notes': 'Zero-mean, unit abs sum by bar'},
    ...
    'cs_rank':    {'category': 'cross_section', 'params': 'x', 'desc': 'Alias for rank(x)',
                   'example': 'cs_rank(volume)', 'notes': 'Percentile rank by bar'},
    'cs_zscore':  {'category': 'cross_section', 'params': 'x', 'desc': 'Alias for zscore(x)',
                   'example': 'cs_zscore(close_trade_px)', 'notes': 'Z-score by bar'},
    'cs_demean':  {'category': 'cross_section', 'params': 'x', 'desc': 'Alias for demean(x)',
                   'example': 'cs_demean(close_trade_px)', 'notes': 'De-mean by bar'},
    ...
    'cs_winsorize': {'category': 'cross_section', 'params': 'x, p', 'desc': 'Alias for winsorize(x,p)',
                     'example': 'cs_winsorize(volume, 0.01)', 'notes': 'Cross-sectional tail clipping'},
    'cs_quantile': {'category': 'cross_section', 'params': 'x, q', 'desc': 'Distance from cross-sectional q-quantile',
                    'example': 'cs_quantile(volume, 0.5)', 'notes': 'Positive above the q-quantile'},
    'cs_neutralize': {'category': 'cross_section', 'params': 'x, y', 'desc': 'Cross-sectional residual of x after projection on y',
                      'example': 'cs_neutralize(delta(close_trade_px,1), volume)', 'notes': 'Useful to remove a liquidity/risk proxy'},
    ...
    'ts_median':  {'category': 'time_series', 'params': 'x, n', 'desc': 'Rolling median over n bars',
                   'example': 'ts_median(vwap, 12)', 'notes': 'Robust local baseline'},
    'ts_quantile': {'category': 'time_series', 'params': 'x, n, q', 'desc': 'Rolling q-quantile over n bars',
                    'example': 'ts_quantile(volume, 20, 0.8)', 'notes': 'Use literal q between 0 and 1'},
    'ts_skew':    {'category': 'time_series', 'params': 'x, n', 'desc': 'Rolling skewness',
                   'example': 'ts_skew(delta(close_trade_px,1), 20)', 'notes': 'Asymmetry of recent moves'},
    'ts_kurt':    {'category': 'time_series', 'params': 'x, n', 'desc': 'Rolling kurtosis',
                   'example': 'ts_kurt(delta(close_trade_px,1), 20)', 'notes': 'Tail intensity of recent moves'},
    'ts_ema':     {'category': 'time_series', 'params': 'x, n', 'desc': 'Exponential moving average',
                   'example': 'ts_ema(close_trade_px, 12)', 'notes': 'Smoother with recent bars weighted more'},
    'ts_argmax':  {'category': 'time_series', 'params': 'x, n', 'desc': 'Bars since rolling maximum position in window',
                   'example': 'ts_argmax(high_trade_px, 20)', 'notes': '1 means oldest item in current rolling implementation window index + 1'},
    'ts_argmin':  {'category': 'time_series', 'params': 'x, n', 'desc': 'Bars since rolling minimum position in window',
                   'example': 'ts_argmin(low_trade_px, 20)', 'notes': 'Window index + 1 of min observation'},
    'ts_pct_change': {'category': 'time_series', 'params': 'x, n', 'desc': 'x / lag(x,n) - 1',
                      'example': 'ts_pct_change(close_trade_px, 1)', 'notes': 'Safe percent change'},
    'ts_minmax_norm': {'category': 'time_series', 'params': 'x, n', 'desc': 'Rolling min-max normalized value',
                       'example': 'ts_minmax_norm(close_trade_px, 20)', 'notes': '(x - min) / (max - min)'},
    ...
    'ts_decay_linear': {'category': 'time_series', 'params': 'x, n', 'desc': 'Alias for decay_linear(x,n)',
                        'example': 'ts_decay_linear(close_trade_px, 10)', 'notes': 'Preferred AutoAlpha spelling'},
    ...
    'clamp':      {'category': 'math', 'params': 'x, a, b', 'desc': 'Alias for clip(x,a,b)',
                   'example': 'clamp(ts_zscore(volume,20), -3, 3)', 'notes': 'Bounds outliers'},
    'neg':        {'category': 'math', 'params': 'x', 'desc': 'Negate: -x',
                   'example': 'neg(delta(close_trade_px, 1))', 'notes': 'Flip sign'},
    'sign':       {'category': 'math', 'params': 'x', 'desc': 'Sign: +1, 0, or -1',
                   'example': 'sign(delta(volume, 5))', 'notes': 'Returns the sign of each element'},
    'min_of':     {'category': 'math', 'params': 'x, y', 'desc': 'Element-wise min',
                   'example': 'min_of(ts_zscore(volume,20), 3)', 'notes': 'Useful for capping one side'},
    'max_of':     {'category': 'math', 'params': 'x, y', 'desc': 'Element-wise max',
                   'example': 'max_of(ts_zscore(volume,20), -3)', 'notes': 'Useful for flooring one side'},
    'sigmoid':    {'category': 'math', 'params': 'x', 'desc': 'Smooth squashing to (0,1)',
                   'example': 'sigmoid(ts_zscore(volume,20))', 'notes': 'Soft threshold'},
    'tanh':       {'category': 'math', 'params': 'x', 'desc': 'Smooth squashing to (-1,1)',
                   'example': 'tanh(ts_zscore(delta(close_trade_px,1),20))', 'notes': 'Soft signed clipping'},
    ...
    'ge':         {'category': 'conditional', 'params': 'x, y', 'desc': 'Greater/equal: x >= y → 1 else 0',
                   'example': 'ge(volume, ts_mean(volume, 20))', 'notes': 'Boolean mask'},
    'le':         {'category': 'conditional', 'params': 'x, y', 'desc': 'Less/equal: x <= y → 1 else 0',
                   'example': 'le(close_trade_px, vwap)', 'notes': 'Boolean mask'},
    'eq':         {'category': 'conditional', 'params': 'x, y', 'desc': 'Approx equality → 1 else 0',
                   'example': 'eq(sign(delta(close_trade_px,1)), 1)', 'notes': 'Epsilon-protected equality'},

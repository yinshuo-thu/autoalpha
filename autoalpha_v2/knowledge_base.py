...
def _embed_texts(texts: list[str], *, model: str | None = None) -> list[list[float]] | None:
    if not texts or _embedding_cooldown_active():
        return None
    ...
    for url in _candidate_embedding_urls(api_base):
        try:
            ...
            continue
    if last_error:
        print(f"[knowledge_base] Embedding request failed, semantic RAG cooldown enabled: {last_error}")
...
def add_factor(
    ...
    if entry.get("PassGates") and (entry.get("submit_path") or entry.get("parquet_path")):
        try:
            from autoalpha_v2.factor_research import build_factor_correlation_matrix

            build_factor_correlation_matrix(max_factors=30, update_cards=True)
        except Exception as exc:
...
def _query_guided_passing_factors(
    ...
    *,
    ...
    _ensure_passing_factor_embeddings(passing)
    store = _load_embedding_store()
    model_name = _embedding_model()

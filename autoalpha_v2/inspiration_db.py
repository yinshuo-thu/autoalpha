...
def normalize_source_type(source_type: Any) -> str:
    ...
    if value in {"arxiv", "openalex", "ssrn", "nber", "scholar"}:
        return "paper"
    if value in {"llm", "future", "paper"}:
        return value
    return "manual"


def _configured_context_sources(preferred_source: Optional[str] = None) -> List[str]:
    configured = os.environ.get("AUTOALPHA_INSPIRATION_SOURCES", "paper,llm,future,manual")
    allowed: List[str] = []
    for raw in configured.split(","):
        source = normalize_source_type(raw.strip())
        if source in {"paper", "llm", "future", "manual"} and source not in allowed:
            allowed.append(source)
    if not allowed:
        allowed = ["paper", "llm", "future", "manual"]
...
def get_balanced_inspiration_context_rows(
    ...
    with _get_conn() as conn:
        for source in sources:
            if source == "manual":
                where = "source_type IN ('manual', 'url', 'wechat')"
                params: tuple[Any, ...] = (per_source + 1,)
            else:
                where = "source_type = ?"
                params = (source, per_source + 1)
            rows = conn.execute(
                f"""
                SELECT * FROM inspirations
...
def list_inspiration_source_counts() -> List[Dict[str, Any]]:
    with _get_conn() as conn:
        rows = conn.execute(
            """
            SELECT CASE WHEN source_type IN ('manual', 'url', 'wechat') THEN 'manual' ELSE source_type END AS source_type,
                   COUNT(*) AS count
            FROM inspirations
            WHERE status = 'active'
            GROUP BY CASE WHEN source_type IN ('manual', 'url', 'wechat') THEN 'manual' ELSE source_type END
            """
        ).fetchall()
    return rows
...
def list_inspirations_paginated(
    ...
    if normalized_source:
        if normalized_source == "manual":
            conditions.append("source_type IN ('manual', 'url', 'wechat')")
        else:
...
def compose_inspiration_context_with_sources(
    ...
    sync_prompt_directory(limit=80)
    rows = get_balanced_inspiration_context_rows(limit=limit, preferred_source=preferred_source)
    if not rows:
        return "", []

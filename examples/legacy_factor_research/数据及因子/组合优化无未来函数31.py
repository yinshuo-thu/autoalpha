...
logger = logging.getLogger(__name__)
def safe_rolling(series, window, func='mean', **kwargs):
    """
    安全的滚动窗口操作，确保不使用未来数据
    ...
    """
    # 强制设置参数确保不使用未来数据
    kwargs['center'] = False  # 强制不使用中心化窗口
    kwargs['min_periods'] = kwargs.get('min_periods', max(1, window//2))
    
    # 创建滚动窗口对象
    roller = series.rolling(window, **kwargs)
    ...
    if func == 'mean':
        ...
        q = kwargs.pop('q', 0.5)
        result = roller.quantile(q)
    elif callable(func):
        # 自定义函数
        result = roller.apply(func, raw=kwargs.pop('raw', False))
    else:
        raise ValueError(f"Unsupported function: {func}")
    
    # 验证：确保结果不包含未来信息
    if hasattr(result, 'index') and hasattr(series, 'index'):
        # 检查索引对齐
        assert result.index.equals(series.index), "索引不匹配，可能存在未来函数"
        
        # 检查前window-1个值应该是NaN（除非设置了min_periods）
        min_periods_actual = kwargs.get('min_periods', window)
        if min_periods_actual == window:
            if isinstance(result, pd.Series):
                assert result.iloc[:window-1].isna().all(), "前期数据应为NaN"
            elif isinstance(result, pd.DataFrame):
                assert result.iloc[:window-1].isna().all().all(), "前期数据应为NaN"
    
    return result


def safe_ewm(series, span=None, alpha=None, adjust=True, **kwargs):
    """
    安全的指数加权移动平均，确保不使用未来数据
    ...
    if span is None and alpha is None:
        raise ValueError("Must specify either span or alpha")
    
    ewm_obj = series.ewm(span=span, alpha=alpha, adjust=adjust, **kwargs)
    return ewm_obj.mean()
...
def calculate_safe_technical_indicators(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for period in [5, 10, 20, 30, 60]:
        ...
        df[f'sma_{period}'] = safe_rolling(df['close'], window=period, 
                                          func='mean', min_periods=min_periods)
        df[f'ema_{period}'] = safe_ewm(df['close'], span=period)
        df[f'volume_sma_{period}'] = safe_rolling(df['volume'], window=period, 
                                                 func='mean', min_periods=min_periods)
    
    # 价格位置特征（使用safe_rolling）
    for period in [10, 20, 30]:
        min_periods = max(period // 2, 5)
        rolling_high = safe_rolling(df['high'], window=period, func='max', 
                                   min_periods=min_periods)
        rolling_low = safe_rolling(df['low'], window=period, func='min', 
                                  min_periods=min_periods)
        df[f'price_position_{period}'] = (df['close'] - rolling_low) / (rolling_high - rolling_low + 1e-10)
    
    # 波动率特征（使用safe_rolling）
    for period in [5, 10, 20]:
        min_periods = max(period // 2, 3)
        df[f'volatility_{period}'] = safe_rolling(df['returns'], window=period, 
                                                 func='std', min_periods=min_periods)
        df[f'volume_volatility_{period}'] = safe_rolling(df['volume'], window=period, 
                                                        func='std', min_periods=min_periods)
    
    # RSI指标（使用safe_rolling）
    for period in [6, 12, 24]:
        delta = df['close'].diff()
        gain = delta.where(delta > 0, 0)
        loss = -delta.where(delta < 0, 0)
        min_periods = max(period // 2, 3)
        avg_gain = safe_rolling(gain, window=period, func='mean', min_periods=min_periods)
        avg_loss = safe_rolling(loss, window=period, func='mean', min_periods=min_periods)
        rs = avg_gain / (avg_loss + 1e-10)
        df[f'rsi_{period}'] = 100 - (100 / (1 + rs))
    
    # MACD指标
    ema_12 = safe_ewm(df['close'], span=12)
    ema_26 = safe_ewm(df['close'], span=26)
    df['macd'] = ema_12 - ema_26
    df['macd_signal'] = safe_ewm(df['macd'], span=9)
    df['macd_diff'] = df['macd'] - df['macd_signal']
    
    # 布林带（使用safe_rolling）
    for period in [10, 20]:
        min_periods = max(period // 2, 5)
        sma = safe_rolling(df['close'], window=period, func='mean', min_periods=min_periods)
        std = safe_rolling(df['close'], window=period, func='std', min_periods=min_periods)
    ...
    for period in [7, 14, 21]:
        ...
        df[f'atr_{period}'] = safe_rolling(true_range, window=period, 
                                          func='mean', min_periods=min_periods)
    
    # OBV（使用safe_rolling）
    obv = (np.sign(df['close'].diff()) * df['volume']).fillna(0).cumsum()
    df['obv'] = obv
    df['obv_sma'] = safe_rolling(obv, window=20, func='mean', min_periods=10)
    
    # 成交量特征（使用safe_rolling）
    volume_ma = safe_rolling(df['volume'], window=20, func='mean', min_periods=10)
    df['volume_ratio'] = df['volume'] / (volume_ma + 1e-10)
    volume_ma_5 = safe_rolling(df['volume'], window=5, func='mean', min_periods=3)
    volume_ma_20 = safe_rolling(df['volume'], window=20, func='mean', min_periods=10)
    df['volume_trend'] = volume_ma_5 / (volume_ma_20 + 1e-10)
...
def validate_and_calculate_extended_factors(df: pd.DataFrame, 
    ...
    original_close = stock_data['close'].copy()  # 保存原始收盘价用于验证
            ...
            if hasattr(factors, func_name):
                ...
                for factor_name in factor_names:
                    if factor_name in stock_data.columns and factor_name not in original_columns:
                        # 严格检查因子是否使用了未来数据
                        factor_series = stock_data[factor_name]
                        
                        # 检查1: 与未来价格的相关性
                        is_suspicious = False
                        for future_lag in range(1, 6):
                            if len(original_close) > future_lag + 100:
                                future_prices = original_close.shift(-future_lag)
                                valid_idx = ~(factor_series.isna() | future_prices.isna())
                                
                                if valid_idx.sum() > 100:
                                    future_corr = factor_series[valid_idx].corr(future_prices[valid_idx])
                                    if abs(future_corr) > 0.6:  # 降低阈值以更严格
                                        logger.warning(f"Factor {factor_name} has high correlation "
                                                     f"({future_corr:.3f}) with {future_lag}-period future prices")
                                        is_suspicious = True
                                        break
                        
                        # 检查2: 因子的"完美性"检查
                        if not is_suspicious and factor_series.notna().sum() > 100:
                            # 检查自相关性
                            autocorr = factor_series.autocorr(lag=1)
                            if pd.notna(autocorr) and autocorr > 0.999:
                                logger.warning(f"Factor {factor_name} has suspiciously high "
                                              f"autocorrelation: {autocorr:.4f}")
                                is_suspicious = True
                            
                            # 检查与当前价格的过高相关性（可能是直接使用了价格）
                            curr_corr = factor_series.corr(original_close)
                            if abs(curr_corr) > 0.995:
                                logger.warning(f"Factor {factor_name} is almost identical to price")
                                is_suspicious = True
                        
                        # 检查3: 前瞻性偏差检查
                        # 检查因子值是否在未来值变化前就"预知"了变化
                        if not is_suspicious and len(factor_series) > 10:
                            factor_diff = factor_series.diff()
                            price_diff = original_close.diff()
                            
                            # 检查因子变化是否领先价格变化
                            for lead in range(1, 4):
                                if len(factor_diff) > lead + 10:
                                    lead_corr = factor_diff.shift(lead).corr(price_diff)
                                    if abs(lead_corr) > 0.5:
                                        logger.warning(f"Factor {factor_name} changes seem to "
                                                     f"predict price changes {lead} periods ahead")
                                        is_suspicious = True
                                        break
                        ...
                        if is_suspicious:
                            ...
                            removed_factors.append(factor_name)
                        else:
    ...
    for col in all_factor_names:
        if col in stock_data.columns:
            ...
            # 逐行处理，确保不使用未来数据
            col_values = stock_data[col].values.copy()
            for i in range(len(col_values)):
                if pd.isna(col_values[i]):
                    # 只使用历史数据填充
                    if i > 0 and not pd.isna(col_values[i-1]):
                        col_values[i] = col_values[i-1]  # 使用前一个值
                    elif i >= 20:
                        # 使用历史均值
                        hist_values = col_values[max(0, i-50):i]
                        hist_clean = hist_values[~pd.isna(hist_values)]
                        if len(hist_clean) > 0:
                            col_values[i] = np.mean(hist_clean)
                        else:
                            col_values[i] = 0
                    else:
                        col_values[i] = 0
            
            stock_data[col] = col_values
    ...
    if removed_factors:
        logger.info(f"Complete list of removed factors: {removed_factors}")
...
def create_trading_target_variable_no_future(df: pd.DataFrame, horizon: int = 5, 
    ...
    if params is None:
        # 默认参数集 - 经过优化的最佳参数
        params = {
            'cost_threshold': 0.0002,      # 交易成本阈值
            'volatility_lookback': 30,     # 波动率回望窗口
            'volatility_multiplier': 0.8,   # 波动率乘数
            'strong_signal_mult': 1.5,     # 强信号倍数
            'weak_signal_mult': 0.8,       # 弱信号倍数
            'min_samples': 20,             # 最小样本数
            'use_dynamic_threshold': True,  # 动态阈值
            'label_smoothing': 0.1,        # 标签平滑
            'trend_weight': 0.2,           # 趋势权重
            'use_multi_horizon': True      # 多时间尺度
        }
    ...
    for i in range(len(df)):
        if i < params['min_samples'] or i >= len(df) - horizon * 2:
            target.iloc[i] = 0
            continue
        
        # 1. 计算自适应波动率
        lookback = params['volatility_lookback']
        hist_returns = close_prices.iloc[max(0, i-lookback):i].pct_change().dropna()
        
        # 添加断言
        if len(hist_returns) > 0:
            assert hist_returns.index[-1] < df.index[i], \
                   f"历史数据包含当前点：{hist_returns.index[-1]} >= {df.index[i]}"
        
        if len(hist_returns) < params['min_samples']:
            continue
        
        # 使用稳健的波动率估计（MAD）
        volatility = hist_returns.std()
        mad_volatility = (hist_returns - hist_returns.median()).abs().median() * 1.4826
        robust_volatility = min(volatility, mad_volatility * 1.2)
        
        # 2. 动态阈值计算
        if params['use_dynamic_threshold']:
            # 基于市场状态的动态阈值
            recent_trend = (close_prices.iloc[i] / close_prices.iloc[max(0, i-10)] - 1)
            trend_adjustment = 1 + abs(recent_trend) * params['trend_weight']
            
            base_threshold = max(
                robust_volatility * params['volatility_multiplier'],
                params['cost_threshold'] * 2
            )
            signal_threshold = base_threshold * trend_adjustment
        else:
        ...
        if i < len(df) - horizon * 2:
            # 计算未来收益时的断言
            assert df.index[i] < df.index[i+horizon], \
                   f"未来数据索引错误：{df.index[i]} >= {df.index[i+horizon]}"
            
            if params['use_multi_horizon']:
                # 多时间尺度收益
                r1 = (close_prices.iloc[i+horizon] / close_prices.iloc[i] - 1)
                r2 = (close_prices.iloc[i+horizon*2] / close_prices.iloc[i] - 1) if i < len(df) - horizon*2 else r1
                
                # 使用加权平均
                future_return = r1 * 0.7 + r2 * 0.3
            else:
                # 单一时间尺度
                future_return = (close_prices.iloc[i+horizon] / close_prices.iloc[i] - 1)
            
            # 4. 生成分级标签
            if abs(future_return) > signal_threshold * params['strong_signal_mult']:
                # 强信号
                label = np.sign(future_return) * 1.0
            elif abs(future_return) > signal_threshold:
                # 中等信号
                label = np.sign(future_return) * 0.6
            elif abs(future_return) > signal_threshold * params['weak_signal_mult']:
                # 弱信号
                label = np.sign(future_return) * 0.3
            else:
                # 噪音区间
                label = 0
            
            # 5. 标签平滑（可选）
            if params['label_smoothing'] > 0 and label != 0:
                label = label * (1 - params['label_smoothing']) + \
                        future_return / signal_threshold * params['label_smoothing']
    ...
    # 统计信息
    non_zero = (target != 0).sum()
    logger.info(f"生成标签统计: 总计{len(target)}个, 有效{non_zero}个({non_zero/len(target)*100:.1f}%)")
    logger.info(f"标签分布: 强买{(target > 0.8).sum()}, 中买{((target > 0.3) & (target <= 0.8)).sum()}, "
                f"弱买{((target > 0) & (target <= 0.3)).sum()}, "
                f"弱卖{((target < 0) & (target >= -0.3)).sum()}, "
                f"中卖{((target < -0.3) & (target >= -0.8)).sum()}, "
                f"强卖{(target < -0.8).sum()}")
        ...
        try:
            ...
            for i in range(len(features_df)):
                if i >= 50 and 'returns' in features_df.columns:
                    ...
                    # 断言：历史数据不包含当前点
                    assert len(hist_returns) <= 50, "历史窗口包含了过多数据"
                    assert hist_returns.index[-1] < features_df.index[i] if len(hist_returns) > 0 else True, \
                           "历史数据包含了当前或未来数据"
            ...
            for i in range(len(predictions)):
                ...
                # 断言：只使用历史数据
                assert i >= self.signal_delay, f"信号生成时索引错误：{i} < {self.signal_delay}"
            ...
            # 平滑信号（只使用历史数据）
            if len(signals) > 3:
                signals = safe_rolling(signals, window=3, func='mean', min_periods=1)
...
class NoFutureBacktestEngine:
   ...
   def run_backtest(self, signals_dict: Dict[str, pd.Series], 
        ...
        for i, date in enumerate(all_dates):
            ...
            # 添加严格的时间断言
            assert signal_date < date, f"未来函数错误：信号日期 {signal_date} 不早于当前日期 {date}"
            assert i == 0 or all_dates[i-1] < all_dates[i], f"时间序列不递增：{all_dates[i-1]} >= {all_dates[i]}"
...
class NoFutureModelTrainer:
    ...
    def prepare_data(self, features_df: pd.DataFrame, targets_df: pd.Series, 
        ...
        # 添加断言确保训练集在测试集之前
        assert features_clean.index[0] < features_clean.index[split_idx-1], \
               "训练集时间范围错误"
        assert features_clean.index[split_idx] < features_clean.index[-1], \
               "测试集时间范围错误"
        assert features_clean.index[split_idx-1] < features_clean.index[split_idx], \
               "训练集和测试集有重叠"
...
def prepare_training_data_for_symbol_no_future(args):
    ...
    try:
        ...
        for idx in range(len(target)):
            if idx < len(target) - look_ahead:
                # 断言：目标值应该基于未来数据
                assert aligned_prices.index[idx] < aligned_prices.index[idx + look_ahead], \
                       f"时间顺序错误：{aligned_prices.index[idx]} >= {aligned_prices.index[idx + look_ahead]}"
...
class NoFutureCommodityStrategy:
   ...
   def prepare_and_train_models(self, look_ahead: int = 3, min_samples: int = 300) -> bool:
        ...
        if len(combined_features) > sample_size:
            ...
            # 修复：使用保持时间顺序的采样方法
            # 选项1：尾部采样（使用最新数据）
            combined_features = combined_features.iloc[-sample_size:]
            combined_targets = combined_targets.iloc[-sample_size:]

...
# ================== 扩展因子计算（整合factors.py）==================
def calculate_extended_factors(df: pd.DataFrame, period_id_list: List[int] = None) -> pd.DataFrame:
    """
    计算扩展因子库中的所有因子
    确保不使用未来数据
    """
    ...
    logger.info(f"Calculating extended factors from factors.py")
    
    # 准备数据格式以适配factors.py的要求
    stock_data = df.copy()
    ...
    if period_id_list is None:
        period_id_list = [3, 4, 5, 6, 7]  # 对应2^3=8, 2^4=16, 2^5=32, 2^6=64, 2^7=128
    
    all_factor_names = []
    
    # 遍历所有可用的因子函数
    print('------------------------------factors.func_list', factors.func_list)
    for func_name in factors.func_list:
        try:
            ...
            if hasattr(factors, func_name):
                ...
                # 调用因子函数
                # 大多数因子函数接受(stock_data, period_id_list)作为参数
                if func_name in ['dev', 'rawret', 'rwi', 'dpo', 'm9', 'cv', 'cr', 'rsi', 'cmo', 
                                 'whlr', 'bop', 'real', 'dis', 'sharp', 'ddi', 'auad', 'qbang', 
                                 'rank', 'hhv', 'tii', 'dmi', 'wvad', 'macd', 'adx', 'vhf', 'mfi',
                                 'kdjd', 'pdi', 'boll', 'adtm', 'abi', 'obv', 'up', 'adjsharp', 
                                 'cjb', 'htb', 'delta', 'dens', 'skew', 'std', 'hl', 'hl2', 'hl3', 
                                 'hl4', 'fluc', 'ch', 'cl', 'lj', 'upvol', 'upvol2', 'vskew',
                                 'active', 'retabs', 'retstd', 'trades', 'volstd', 'volskew', 'volratio']:
                    # 这些是单周期因子，可能不需要period_id_list
                    try:
                        stock_data, factor_names = func(stock_data)
                        all_factor_names.extend(factor_names if isinstance(factor_names, list) else [factor_names])
                    except:
                        # 如果失败，尝试带period_id_list参数
                        try:
                            stock_data, factor_names = func(stock_data, period_id_list)
                            all_factor_names.extend(factor_names if isinstance(factor_names, list) else [factor_names])
                        except Exception as e:
                            logger.debug(f"Failed to calculate {func_name}: {e}")
                else:
                    # 多周期因子
                    stock_data, factor_names = func(stock_data, period_id_list)
                    all_factor_names.extend(factor_names if isinstance(factor_names, list) else [factor_names])
    ...
    logger.info(f"Calculated {len(all_factor_names)} extended factors")
...
def calculate_safe_technical_indicators(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # 移动平均线（只使用历史数据，增加min_periods保证稳定性）
    for period in [5, 10, 20, 30, 60]:
        min_periods = max(period // 2, 3)  # 确保有足够的数据点
        df[f'sma_{period}'] = df['close'].rolling(window=period, min_periods=min_periods).mean()
        df[f'ema_{period}'] = df['close'].ewm(span=period, adjust=False).mean()
        df[f'volume_sma_{period}'] = df['volume'].rolling(window=period, min_periods=min_periods).mean()
    
    # 价格位置特征
    for period in [10, 20, 30]:
        min_periods = max(period // 2, 5)
        rolling_high = df['high'].rolling(window=period, min_periods=min_periods).max()
        rolling_low = df['low'].rolling(window=period, min_periods=min_periods).min()
        df[f'price_position_{period}'] = (df['close'] - rolling_low) / (rolling_high - rolling_low + 1e-10)
    
    # 波动率特征（使用历史数据，增加min_periods）
    for period in [5, 10, 20]:
        min_periods = max(period // 2, 3)
        df[f'volatility_{period}'] = df['returns'].rolling(window=period, min_periods=min_periods).std()
        df[f'volume_volatility_{period}'] = df['volume'].rolling(window=period, min_periods=min_periods).std()
    
    # RSI指标（增加min_periods确保稳定性）
    ...
    for period in [10, 20]:
        min_periods = max(period // 2, 5)
        sma = df['close'].rolling(window=period, min_periods=min_periods).mean()
        std = df['close'].rolling(window=period, min_periods=min_periods).std()
    ...
    for period in [7, 14, 21]:
        ...
        df[f'atr_{period}'] = true_range.rolling(window=period, min_periods=min_periods).mean()
    
    # OBV（能量潮指标）
    obv = (np.sign(df['close'].diff()) * df['volume']).fillna(0).cumsum()
    df['obv'] = obv
    df['obv_sma'] = obv.rolling(window=20, min_periods=10).mean()
    
    # 成交量特征
    df['volume_ratio'] = df['volume'] / df['volume'].rolling(window=20, min_periods=10).mean()
    df['volume_trend'] = df['volume'].rolling(window=5, min_periods=3).mean() / df['volume'].rolling(window=20, min_periods=10).mean()
    ...
    # 现在添加扩展因子库
    df = calculate_extended_factors(df)
...
# ================== 完全无未来函数的数据清理 ==================
def clean_numeric_data_no_future(df: pd.DataFrame, columns: List[str] = None, lookback_window: int = 100) -> pd.DataFrame:
    """
    安全的数值数据清理函数，不使用未来数据
    使用滚动窗口计算统计量
    """
    ...
    for col in columns:
        if col in df.columns:
            ...
            # 使用滚动窗口计算统计量进行异常值处理
            if df[col].notna().sum() > lookback_window:
                # 使用滚动窗口计算分位数，增加min_periods确保稳定性
                min_periods = max(lookback_window // 2, 20)
                rolling_q05 = df[col].rolling(window=lookback_window, min_periods=min_periods).quantile(0.05)
                rolling_q95 = df[col].rolling(window=lookback_window, min_periods=min_periods).quantile(0.95)
                
                # 逐行处理，确保只使用历史数据
                for i in range(len(df)):
                    if i >= lookback_window:
                        if pd.notna(rolling_q05.iloc[i]) and pd.notna(rolling_q95.iloc[i]):
                            if rolling_q05.iloc[i] != rolling_q95.iloc[i]:
                                # 使用历史窗口的分位数裁剪当前值
                                df.loc[df.index[i], col] = np.clip(
                                    df.loc[df.index[i], col],
                                    rolling_q05.iloc[i],
                                    rolling_q95.iloc[i]
                                )
            
            # 使用滚动均值填充缺失值（只使用历史数据，增加min_periods）
            min_periods = max(lookback_window // 4, 10)
            rolling_mean = df[col].rolling(window=lookback_window, min_periods=min_periods).mean()
...
class DataLeakageChecker:
    ...
    @staticmethod
    def check_prediction_leakage(predictions: np.ndarray, targets: np.ndarray, threshold: float = 0.5) -> bool:
        """
        检查预测是否存在数据泄露
        如果相关性过高，可能存在未来函数
        """
        ...
        if correlation > threshold:
            logger.warning(f"警告：预测相关性异常高 ({correlation:.4f})，可能存在数据泄露！")
            return True
        
        logger.info(f"预测相关性正常: {correlation:.4f}")
        return False
    
    @staticmethod
    def check_feature_target_leakage(features: pd.DataFrame, target: pd.Series, threshold: float = 0.8) -> List[str]:
        """
        检查特征与目标之间是否存在异常高的相关性
        返回可疑特征列表
        """
        suspicious_features = []
        ...
        for col in features.columns:
            if features[col].std() > 0:
                ...
                if abs(correlation) > threshold:
                    logger.warning(f"特征 {col} 与目标相关性过高: {correlation:.4f}")
                    suspicious_features.append(col)
        
        return suspicious_features
    
    @staticmethod
    def check_temporal_consistency(data: pd.DataFrame, date_column: str = None) -> bool:
        """
        检查时间序列的一致性
        确保数据按时间顺序排列，没有未来数据混入
        """
        if date_column and date_column in data.columns:
            dates = pd.to_datetime(data[date_column])
            if not dates.is_monotonic_increasing:
                logger.warning("警告：数据时间序列不是单调递增的，可能存在时间混乱！")
                return False
        elif isinstance(data.index, pd.DatetimeIndex):
            if not data.index.is_monotonic_increasing:
                logger.warning("警告：数据索引不是单调递增的，可能存在时间混乱！")
                return False
        
        return True

# ================== 无未来函数的目标变量创建 ==================
def create_trading_target_variable_no_future(df: pd.DataFrame, horizon: int = 3) -> pd.Series:
    """
    创建交易目标变量（这里使用未来数据是合理的，因为是训练标签）
    但要确保在实际交易时不会使用这些未来数据
    """
    logger.info(f"创建目标变量，数据长度: {len(df)}, 预测周期: {horizon}")
    
    # 计算未来收益率（这是我们要预测的目标）
    close_prices = df['close'].copy()
    future_prices = close_prices.shift(-horizon)
    future_return = (future_prices / close_prices) - 1
    
    # 清理目标变量
    future_return = future_return.replace([np.inf, -np.inf], np.nan)
    
    # 使用滚动窗口的分位数进行异常值处理
    if len(future_return.dropna()) > 100:
        # 使用历史数据的分位数
        min_periods = 50
        rolling_q01 = future_return.rolling(window=100, min_periods=min_periods).quantile(0.01)
        rolling_q99 = future_return.rolling(window=100, min_periods=min_periods).quantile(0.99)
        
        for i in range(len(future_return)):
            if i >= 100:
                if pd.notna(rolling_q01.iloc[i]) and pd.notna(rolling_q99.iloc[i]):
                    future_return.iloc[i] = np.clip(
    ...
    # 标准化目标变量（使用滚动窗口）
    min_periods = 50
    rolling_std = future_return.rolling(window=100, min_periods=min_periods).std()
    for i in range(len(future_return)):
        if i >= 100 and rolling_std.iloc[i] > 0:
            future_return.iloc[i] = future_return.iloc[i] / rolling_std.iloc[i]
    
    # 最后处理缺失值
    future_return = future_return.fillna(0)
    
    logger.info(f"目标变量统计: 均值={future_return.mean():.6f}, 标准差={future_return.std():.6f}")
    logger.info(f"非零目标变量数量: {(future_return != 0).sum()}")
...
class NoFutureBacktestEngine:
    ...
    def run_backtest(self, signals_dict: Dict[str, pd.Series], 
        ...
        for symbol, data in price_data_dict.items():
            if not leakage_checker.check_temporal_consistency(data):
...
class NoFutureSignalGenerator:
    ...
    def __init__(self, model_trainer):
        ...
        self.signal_delay = 1  # 信号延迟，避免使用当前K线
    ...
    def _prepare_features(self, features_df, feature_cols):
        ...
        # 使用滚动窗口的中位数填充（增加min_periods）
        for col in X.columns:
            rolling_median = X[col].rolling(window=50, min_periods=10).median()
    ...
    def plot_results(self, backtest_results: Dict, save_path: str = None):
        ...
        if len(portfolio_df) > 0:
            ...
            if len(portfolio_df) > 60:
                rolling_returns = portfolio_df['portfolio_value'].pct_change().rolling(60)
...
def process_single_symbol_data_no_future(args):
    ...
    try:
        ...
        cache_params = {'sample_rate': sample_rate, 'version': 'no_future_extended_v1'}
...
def process_features_for_symbol_no_future(args):
    ...
    try:
        ...
        cache_params = {'version': 'no_future_extended_v1', 'factors_available': FACTORS_AVAILABLE}
        ...
        for col in feature_cols:
            if col in data.columns:
                ...
                # 使用滚动窗口填充，增加min_periods
                rolling_median = data[col].rolling(window=50, min_periods=20).median()
...
class NoFutureCommodityStrategy:
    ...
    def __init__(self, data_path: str, info_path: str, use_gpu: bool = True, 
        ...
        self.model_trainer = NoFutureModelTrainer(use_gpu=use_gpu)
...
if __name__ == "__main__":
    print("Starting Complete No-Future-Function strategy with Extended Factors...")
    print("\n主要改进:")
    print("1. 集成autodl-tmp/factors/factors.py中的100+技术因子")
    print("2. 动态特征选择，根据因子数量调整")
    print("3. 扩展缓存系统支持新因子")
    print("4. 所有因子计算确保无未来函数")
    print("5. 优化内存管理支持更多特征\n")

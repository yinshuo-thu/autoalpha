...
class CrossSectionalFeatureCalculator:
    ...
    def calculate_cross_sectional_features(self, 
        ...
        """
        # 获取所有时间点
        all_dates = sorted(list(set().union(*[set(df.index) for df in aligned_data.values()])))
        
        # 初始化结果存储
        cross_sectional_features = []
        
        logger.info(f"计算截面特征，时间点数量: {len(all_dates)}, 品种数量: {len(aligned_data)}")
        
        # 逐个时间点计算截面特征
        for i, current_date in enumerate(tqdm(all_dates, desc="计算截面特征")):
            if i < lookback_window:
                continue  # 需要足够的历史数据
            
            # 获取历史窗口（不包括当前点，避免未来函数）
            hist_dates = all_dates[max(0, i-lookback_window):i]
            
            # 计算每个品种在当前时间点的特征
            date_features = self._calculate_features_at_date(
                aligned_data, current_date, hist_dates
            )
            
            if date_features:
                cross_sectional_features.extend(date_features)
        
        # 转换为DataFrame
        if cross_sectional_features:
            result_df = pd.DataFrame(cross_sectional_features)
            logger.info(f"截面特征计算完成，生成 {len(result_df)} 条记录")
            return result_df
        else:
            logger.warning("未生成任何截面特征")
            return pd.DataFrame()
    
    def _calculate_features_at_date(self, 
                                   aligned_data: Dict[str, pd.DataFrame],
                                   current_date: pd.Timestamp,
                                   hist_dates: List[pd.Timestamp]) -> List[Dict]:
        """计算特定日期的截面特征"""
        features_list = []
        ...
        for symbol, df in aligned_data.items():
            if current_date in df.index and len(hist_dates) > 0:
                # 计算历史收益率（只使用历史数据）
                hist_prices = []
                for hd in hist_dates:
                    if hd in df.index:
                        hist_prices.append(df.loc[hd, 'close'])
                
                if len(hist_prices) >= 10:  # 需要足够的历史数据
                    returns = pd.Series(hist_prices).pct_change().dropna()
                    
                    current_data[symbol] = {
                        'close': df.loc[current_date, 'close'],
                        'volume': df.loc[current_date, 'volume'],
                        'returns': returns.iloc[-1] if len(returns) > 0 else 0,
                        'mean_return': returns.mean(),
                        'volatility': returns.std(),
                        'momentum': (hist_prices[-1] / hist_prices[0] - 1) if hist_prices[0] != 0 else 0
                    }
                    
                    # 保存原始特征（如果存在）
                    feature_cols = [col for col in df.columns 
                                  if col not in ['open', 'high', 'low', 'close', 'volume']]
                    for col in feature_cols[:50]:  # 限制特征数量
                        if col in df.columns:
                            current_data[symbol][col] = df.loc[current_date, col]
        
        if len(current_data) < 2:
            return []  # 需要至少2个品种才能计算截面特征
        
        # 计算截面统计量
        symbols = list(current_data.keys())
        returns_list = [current_data[s]['returns'] for s in symbols]
        momentum_list = [current_data[s]['momentum'] for s in symbols]
        volatility_list = [current_data[s]['volatility'] for s in symbols]
        volume_list = [current_data[s]['volume'] for s in symbols]
        
        # 计算排名和分位数
        returns_rank = pd.Series(returns_list).rank(pct=True)
        momentum_rank = pd.Series(momentum_list).rank(pct=True)
        volatility_rank = pd.Series(volatility_list).rank(pct=True)
        volume_rank = pd.Series(volume_list).rank(pct=True)
        
        # 计算截面均值和标准差
        returns_mean = np.mean(returns_list)
        returns_std = np.std(returns_list)
        momentum_mean = np.mean(momentum_list)
        momentum_std = np.std(momentum_list)
        ...
        for idx, symbol in enumerate(symbols):
            features = {
                ...
                # 原始特征
                'close': current_data[symbol]['close'],
                'volume': current_data[symbol]['volume'],
                'returns': current_data[symbol]['returns'],
                'momentum': current_data[symbol]['momentum'],
                'volatility': current_data[symbol]['volatility'],
                
                # 截面排名特征
                'returns_rank': returns_rank.iloc[idx],
                'momentum_rank': momentum_rank.iloc[idx],
                'volatility_rank': volatility_rank.iloc[idx],
                'volume_rank': volume_rank.iloc[idx],
                
                # 相对特征（标准化）
                'returns_zscore': (current_data[symbol]['returns'] - returns_mean) / (returns_std + 1e-10),
                'momentum_zscore': (current_data[symbol]['momentum'] - momentum_mean) / (momentum_std + 1e-10),
                
                # 截面分散度
                'cross_dispersion': returns_std,
                'cross_momentum_dispersion': momentum_std,
            ...
            # 添加原始特征
            for col in current_data[symbol]:
                if col not in features and not col.startswith('returns') and not col.startswith('momentum'):
                    features[f'raw_{col}'] = current_data[symbol][col]
...
# ================== 截面数据对齐器 ==================
class DataAligner:
    """数据对齐器，确保所有品种在相同时间点上对齐"""
    
    @staticmethod
    def align_multi_symbol_data(data_dict: Dict[str, pd.DataFrame], 
                               min_symbols_per_date: int = 3) -> Dict[str, pd.DataFrame]:
        """
        对齐多个品种的数据
        
        Args:
            data_dict: 品种数据字典
            min_symbols_per_date: 每个时间点最少需要的品种数
            
        Returns:
            对齐后的数据字典
        """
        logger.info(f"开始对齐 {len(data_dict)} 个品种的数据")
        ...
        logger.info(f"总共 {len(all_dates)} 个时间点")
        
        # 统计每个时间点有多少品种有数据
        date_coverage = {}
        for date in all_dates:
            count = sum(1 for df in data_dict.values() if date in df.index)
            date_coverage[date] = count
        
        # 筛选出有足够品种的时间点
        valid_dates = [date for date, count in date_coverage.items() 
                      if count >= min_symbols_per_date]
        
        logger.info(f"有效时间点（至少{min_symbols_per_date}个品种）: {len(valid_dates)}")
        
        # 对每个品种，只保留有效时间点的数据
        aligned_data = {}
        for symbol, df in data_dict.items():
            valid_index = [date for date in valid_dates if date in df.index]
            if len(valid_index) > 100:  # 至少需要100个数据点
                aligned_data[symbol] = df.loc[valid_index].copy()
                logger.info(f"{symbol}: 保留 {len(valid_index)} 个时间点")
        
        logger.info(f"对齐完成，保留 {len(aligned_data)} 个品种")
        return aligned_data

# ================== 截面训练数据准备 ==================
class CrossSectionalDataPreparer:
    """准备截面训练数据"""
    
    def __init__(self):
        self.feature_calculator = CrossSectionalFeatureCalculator()
        self.data_aligner = DataAligner()
    
    def prepare_cross_sectional_training_data(self,
                                             data_dict: Dict[str, pd.DataFrame],
                                             features_dict: Dict[str, Tuple[pd.DataFrame, List[str]]],
                                             lookback: int = 20,
        ...
        """
        logger.info("准备截面训练数据...")
        
        # 1. 数据对齐
        aligned_data = self.data_aligner.align_multi_symbol_data(data_dict)
        ...
        for symbol in aligned_data:
            if symbol in features_dict:
                ...
                # 合并特征到原始数据
                common_index = aligned_data[symbol].index.intersection(features_df.index)
                for col in feature_cols[:30]:  # 限制特征数量
                    if col in features_df.columns:
                        aligned_data[symbol].loc[common_index, col] = features_df.loc[common_index, col]
        
        # 3. 计算截面特征
        cross_features_df = self.feature_calculator.calculate_cross_sectional_features(
            aligned_data, lookback_window=lookback
        )
        
        if cross_features_df.empty:
            logger.error("截面特征计算失败")
            return pd.DataFrame(), pd.Series()
        
        # 4. 创建目标变量（基于未来收益率的截面排名）
        targets = self._create_cross_sectional_targets(
            aligned_data, cross_features_df, lookahead
        )
        
        # 5. 清理和对齐数据
        common_index = cross_features_df.index.intersection(targets.index)
        
        X = cross_features_df.loc[common_index]
        y = targets.loc[common_index]
        
        # 移除datetime和symbol列作为特征
        feature_cols = [col for col in X.columns 
                       if col not in ['datetime', 'symbol']]
        
        logger.info(f"生成训练数据: {len(X)} 样本, {len(feature_cols)} 个特征")
        
        return X[feature_cols], y
    
    def _create_cross_sectional_targets(self,
                                       aligned_data: Dict[str, pd.DataFrame],
                                       cross_features_df: pd.DataFrame,
                                       lookahead: int) -> pd.Series:
        """创建截面目标变量（基于未来收益的相对排名）"""
        logger.info(f"创建截面目标变量，预测窗口: {lookahead}")
        
        targets = []
        
        # 按时间分组
        grouped = cross_features_df.groupby('datetime')
                    ...
                    if date in df.index:
                        ...
                        if current_idx + lookahead < len(df):
                            ...
                            if current_price > 0:
                                ...
                                future_returns[symbol] = future_return
            
            # 基于未来收益生成标签
            if len(future_returns) >= 2:
                # 计算排名
                returns_series = pd.Series(future_returns)
                ranks = returns_series.rank(pct=True)
                
                # 生成多空标签
                for symbol, rank in ranks.items():
                    # 排名前30%为1（做多），后30%为-1（做空），中间为0
                    if rank > 0.7:
                        label = 1.0
                    elif rank < 0.3:
                        label = -1.0
                    ...
                    # 找到对应的索引
                    idx = group[group['symbol'] == symbol].index[0]
                    targets.append((idx, label))
        
        # 转换为Series
        if targets:
            target_series = pd.Series(dict(targets))
            logger.info(f"生成 {len(target_series)} 个目标标签")
            logger.info(f"标签分布: 做多 {(target_series > 0).sum()}, "
                       f"做空 {(target_series < 0).sum()}, "
                       f"中性 {(target_series == 0).sum()}")
            return target_series
        else:
            logger.warning("未生成任何目标标签")
            return pd.Series()
...
class CrossSectionalSignalGenerator:
    ...
    def __init__(self, model):
        self.model = model
        self.top_pct = 0.3  # 做多前30%
        self.bottom_pct = 0.3  # 做空后30%
    
    def generate_cross_sectional_signals(self,
        ...
        ranks = pred_series.rank(pct=True)
        
        # 生成信号
        signals = {}
        n_symbols = len(symbols)
        n_long = max(1, int(n_symbols * self.top_pct))
        n_short = max(1, int(n_symbols * self.bottom_pct))
        
        # 选择做多和做空的品种
        top_symbols = pred_series.nlargest(n_long).index
        bottom_symbols = pred_series.nsmallest(n_short).index
        
        for symbol in symbols:
            if symbol in top_symbols:
                # 做多信号，强度根据排名
                signals[symbol] = ranks[symbol]
            elif symbol in bottom_symbols:
                # 做空信号，强度根据排名
                signals[symbol] = -(1 - ranks[symbol])
            else:
                signals[symbol] = 0.0
...
# ================== 主策略修改 ==================
def integrate_cross_sectional_training(strategy_instance):
    """
    将截面训练集成到现有策略中
    这个函数修改您现有的策略类，添加截面训练功能
    """
    # 添加截面组件
    strategy_instance.cross_preparer = CrossSectionalDataPreparer()
    strategy_instance.cross_signal_generator = None  # 训练后初始化
    
    # 修改训练方法
    original_train = strategy_instance.prepare_and_train_models
    
    def new_prepare_and_train_models(look_ahead: int = 5, min_samples: int = 300):
        """截面训练版本"""
        logger.info("="*60)
        logger.info("准备截面训练数据...")

        # 准备截面数据
        X, y = strategy_instance.cross_preparer.prepare_cross_sectional_training_data(
        ...
        # 保存缩放器
        strategy_instance.model_trainer.scaler = scaler
        strategy_instance.model_trainer.scaler_fitted = True
        strategy_instance.model_trainer.selected_features = X_train.columns.tolist()

        # 清理目标变量
        y_train_clean = y_train.values
        y_test_clean = y_test.values

        # 训练模型
        results = strategy_instance.model_trainer.train_ensemble_models(
            X_train_scaled, 
            X_test_scaled,
            y_train_clean, 
            y_test_clean
        ...
        # 初始化截面信号生成器
        strategy_instance.cross_signal_generator = CrossSectionalSignalGenerator(
            strategy_instance.model_trainer
        )

        logger.info("截面模型训练完成!")
    ...
    # 替换原方法
    strategy_instance.prepare_and_train_models = new_prepare_and_train_models
    
    # 修改信号生成
    def generate_cross_sectional_signals():
        """生成截面信号"""
        logger.info("生成截面交易信号...")
        
        # 获取最新的截面特征
        aligned_data = strategy_instance.cross_preparer.data_aligner.align_multi_symbol_data(
            strategy_instance.min30_data
        )
        
        # 为每个时间点生成信号
        all_signals = {symbol: pd.Series(dtype=float) for symbol in aligned_data.keys()}
        
        # 获取所有时间点
        all_dates = sorted(list(set().union(*[set(df.index) for df in aligned_data.values()])))
        
        # 逐个时间点生成信号
        for date in tqdm(all_dates[20:], desc="生成信号"):  # 跳过前20个点
            # 收集当前时间点的特征
            date_features = []
            date_symbols = []
            ...
            for symbol, df in aligned_data.items():
                if date in df.index:
                    # 这里简化处理，实际应该使用完整的特征
                    features = df.loc[date].values
                    date_features.append(features)
                    date_symbols.append(symbol)
            
            if len(date_features) >= 2:
                # 生成截面信号
                signals = strategy_instance.cross_signal_generator.generate_cross_sectional_signals(
                ...
                for symbol, signal in signals.items():
                    if symbol not in all_signals:
                        all_signals[symbol] = pd.Series(dtype=float)
                    all_signals[symbol].loc[date] = signal
        
        # 更新策略的信号
        strategy_instance.signals = all_signals
        
        logger.info(f"生成信号完成，共 {len(all_signals)} 个品种")
        
    # 添加新方法
    strategy_instance.generate_cross_sectional_signals = generate_cross_sectional_signals
...
if __name__ == "__main__":
    ...
    # 集成截面训练功能
    strategy = integrate_cross_sectional_training(strategy)

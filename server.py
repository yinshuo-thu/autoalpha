...
from autoalpha_v3.error_utils import humanize_error
...
from autoalpha_v3.llm_client import summarize_inspiration_text
...
AUTOALPHA_LOG_PATH = os.path.join(PROJECT_ROOT, "loop.log")
AUTOALPHA_WATCHDOG_LOG_PATH = os.path.join(PROJECT_ROOT, "v3_mining.out")
AUTOALPHA_WATCHDOG_PID_PATH = os.path.join(PROJECT_ROOT, "v3_watchdog.pid")
AUTOALPHA_SCREEN_NAME = "autoalpha_v3_mining"
AUTOALPHA_LOOP_PID_PATH = os.path.join(PROJECT_ROOT, "loop.pid")
AUTOALPHA_LOOP_META_PATH = os.path.join(PROJECT_ROOT, "loop_state.json")
AUTOALPHA_KB_PATH  = os.path.join(PROJECT_ROOT, "knowledge.json")
AUTOALPHA_OUTPUT_DIR = os.path.join(PROJECT_ROOT, "output")
AUTOALPHA_RESEARCH_DIR = os.path.join(PROJECT_ROOT, "research")
AUTOALPHA_SUBMIT_DIR = os.path.join(PROJECT_ROOT, "submit")
...
AUTOALPHA_APP_BASE = _normalize_app_base(os.environ.get("AUTOALPHA_APP_BASE", "/v3"))
...
def _resolve_autoalpha_target_valid(requested_value: Any, cfg: Dict[str, Any]) -> int:
    ...
    try:
        from autoalpha_v3.knowledge_base import get_summary as get_autoalpha_summary
...
def _external_autoalpha_loop_pid() -> Optional[int]:
    """Detect autoalpha_v3 mining launched outside Flask, e.g. screen/watchdog."""
    try:
        output = subprocess.check_output(
            ["pgrep", "-f", r"python.*-m autoalpha_v3\.loop"],
            text=True,
        ...
        pass
    try:
        with open(AUTOALPHA_WATCHDOG_PID_PATH, "r", encoding="utf-8") as handle:
            pid = int((handle.read() or "0").strip())
        if _pid_is_running(pid):
            return pid
    except Exception:
...
def _screen_session_running(name: str) -> bool:
    try:
...
def _read_log_tail(paths: List[str], limit: int = 240) -> List[str]:
    chosen = ""
    newest_mtime = -1.0
    for path in paths:
        try:
...
def _parse_autoalpha_live_state(logs: List[str]) -> Dict[str, Any]:
    state: Dict[str, Any] = {
        "phase": "idle",
        "message": "等待启动",
        "round": None,
    ...
    for line in logs:
        if "autoalpha_v3 mining start" in line or "AutoAlpha Mining Loop" in line:
            state.update({"phase": "starting", "message": "挖掘循环启动中"})
        match = re.search(r"--- ROUND\s+([^ ]+)\s+---", line)
        if match:
            label = match.group(1)
            state.update({"phase": "round", "round_label": label, "message": f"第 {label} 轮运行中"})
            try:
                state["round"] = int(label.split("/", 1)[0])
            except Exception:
                pass
        if "[pipeline] Loading DataHub" in line:
            state.update({"phase": "loading_data", "message": "正在加载/构建行情缓存"})
        if "[pipeline] Data window:" in line:
            state["data_window"] = line.split("Data window:", 1)[-1].strip()
            state.update({"phase": "loading_data", "message": "正在准备数据窗口"})
        if "[PREPARE] Precomputing" in line:
            state.update({"phase": "precomputing_cache", "message": line.strip()})
        match = re.search(r"\[(\d+)/(\d+)\]\s+([0-9-]+).*ETA=(\d+)s", line)
        if match:
            current = int(match.group(1))
            total = int(match.group(2))
            progress = round(current / max(total, 1) * 100, 1)
            state.update({
                "phase": "precomputing_cache",
                "message": f"正在构建行情缓存：{current}/{total}，ETA {match.group(4)}s",
                "progress": progress,
                "progress_current": current,
                "progress_total": total,
                "eta_seconds": int(match.group(4)),
            })
        if "[CACHE] Loaded" in line:
            state.update({"phase": "data_ready", "message": line.strip()})
        if "[pipeline] Discovery window:" in line:
            state["eval_window"] = line.split("Discovery window:", 1)[-1].strip()
        if "[pipeline] Eval window:" in line:
            state["eval_window"] = line.split("Eval window:", 1)[-1].strip()
        if "[pipeline] Screen window:" in line:
            state["screen_window"] = line.split("Screen window:", 1)[-1].strip()
        if "[pipeline] OOS report-only window:" in line:
            state["oos_window"] = line.split("OOS report-only window:", 1)[-1].strip()
        if "[pipeline] Export parquet window:" in line:
            state["export_window"] = line.split("Export parquet window:", 1)[-1].strip()
        match = re.search(r"\[pipeline\] Generating idea\s+(\d+)/(\d+)", line)
        if match:
            state.update({
                "phase": "generating",
                "idea": int(match.group(1)),
                "ideas_total": int(match.group(2)),
                "message": f"正在生成 idea {match.group(1)}/{match.group(2)}",
            })
        match = re.search(r"\[pipeline\] Idea\s+(\d+)/(\d+)", line)
        if match:
            state.update({
                "phase": "idea_ready",
                "idea": int(match.group(1)),
                "ideas_total": int(match.group(2)),
                "message": f"idea {match.group(1)}/{match.group(2)} 已生成",
            })
        if "[pipeline] Processing idea" in line:
            formula = line.split(":", 1)[-1].strip()
            state.update({"phase": "computing", "message": "正在计算候选因子", "current_formula": formula})
        if "[Recall]" in line:
            state["last_recall"] = line.strip()
        if "[screen-compute]" in line:
            state.update({"phase": "screen_compute", "message": "正在进行快筛计算"})
        if "[screen-metrics]" in line:
            ...
            pass_match = re.search(r"PassGates=(True|False)", line)
            if pass_match:
                metrics["PassGates"] = pass_match.group(1) == "True"
            state.update({"phase": "screen_metrics", "message": "快筛指标已更新", "last_metrics": metrics})
        if "[screen] Rejected" in line:
            state.update({"phase": "screened_out", "message": "候选因子快筛淘汰", "last_rejection": line.strip()})
        if "Full-window valid factors" in line:
            state.update({"phase": "round_done", "message": line.strip()})
        if "Target reached:" in line:
            state.update({"phase": "target_reached", "message": line.strip()})
        if "exited code=" in line:
            state.update({"phase": "restarting", "message": line.strip()})
    return state
...
def _compact_model_lab_summary(summary: Optional[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    ...
    for model_name, payload in (summary.get("models") or {}).items():
        ...
        curve_limits = {
            "cumulative_curve": 366,
            "drawdown_curve": 366,
            "daily_pnl_curve": 366,
            "long_only_cumulative_curve": 366,
            "long_only_drawdown_curve": 366,
            "long_only_gross_cumulative_curve": 366,
            "long_only_fee_cumulative_curve": 366,
            "daily_long_pnl_curve": 366,
...
def _build_autoalpha_inspiration_stats(all_factors: List[Dict[str, Any]]) -> Dict[str, Any]:
    tracked_sources = ["systematic", "manual", "paper", "llm"]
    ...
    return {
        ...
        "source_mix_note": (
            "Systematic 是 v3 冷启动候选矿工；Manual/Paper/LLM 是灵感库驱动来源。"
            "当 Paper 或 LLM tested_count 偏低时，应提高 inspiration fetch / LLM loop 配额，而不是把 Systematic 计入 Manual。"
        ),
    }


def _load_autoalpha_correlation_cache() -> Dict[str, Any]:
    cache_path = os.path.join(PROJECT_ROOT, "factor_correlations.json")
        ...
        response["factors"] = [
            {
                ...
                "oss_2024": factor.get("oss_2024"),
                "full_2022_2024": factor.get("full_2022_2024"),
                "period_metrics": factor.get("period_metrics", []),
            }
            for factor in all_factors
        ]
...
def autoalpha_generation_experience(generation: int):
    try:
        ...
        if request.method == "POST":
            from autoalpha_v3.llm_client import summarize_generation_experience
...
def _load_autoalpha_research_report(run_id: str, full: bool = False) -> Dict[str, Any]:
    report_path = os.path.join(
        PROJECT_ROOT, "research", run_id, "report.json"
    )
    ...
    card_path = os.path.join(
        PROJECT_ROOT, "research", run_id, "factor_card.json"
    )
...
@app.route("/api/autoalpha_v3/submit/sync", methods=["POST"])
def autoalpha_submit_sync():
    try:
        from autoalpha_v3 import knowledge_base as kb

        return ok(kb.sync_submit_artifacts(), message="可提交因子已同步到 autoalpha_v3/submit")
    except Exception as exc:
        friendly, _, _, raw = humanize_error(exc)
...
def autoalpha_fetch_inspirations():
    ...
    try:
        from autoalpha_v3.inspiration_fetcher import run_fetch_cycle
...
def autoalpha_idea_cache_status():
    try:
        from autoalpha_v3.idea_cache import get_default_cache
...
def autoalpha_loop_start():
    ...
    loop_script = os.path.join(PROJECT_ROOT, "loop.py")
...
def autoalpha_loop_stop():
    ...
    external_pid = _external_autoalpha_loop_pid()
    if pid is not None:
        try:
            if _autoalpha_loop_process is not None and _autoalpha_loop_process.poll() is None:
    ...
    if external_pid is not None or _screen_session_running(AUTOALPHA_SCREEN_NAME):
        try:
            subprocess.run(["screen", "-S", AUTOALPHA_SCREEN_NAME, "-X", "quit"], check=False)
        except Exception:
            pass
        if external_pid is not None:
            try:
                os.kill(external_pid, 15)
            except Exception:
                pass
        return ok({"status": "stopped", "pid": external_pid}, message="外部挖掘循环已停止")
...
def autoalpha_loop_status():
    global _autoalpha_loop_process
    running_pid = _tracked_autoalpha_loop_pid()
    external_pid = None if running_pid is not None else _external_autoalpha_loop_pid()
    is_running = running_pid is not None or external_pid is not None or _screen_session_running(AUTOALPHA_SCREEN_NAME)

    # Read recent log lines
    logs = _read_log_tail([AUTOALPHA_WATCHDOG_LOG_PATH, AUTOALPHA_LOG_PATH], limit=240)
    live_state = _parse_autoalpha_live_state(logs)

    # Knowledge base summary
    kb = _load_autoalpha_kb()
    return ok({
        "is_running": is_running,
        "pid": running_pid or external_pid,
        "process_source": "flask" if running_pid is not None else ("external" if external_pid is not None else "screen" if is_running else "none"),
        "run_state": _read_autoalpha_loop_meta(),
        "total_tested": kb.get("total_tested", 0),
        "total_passing": kb.get("total_passing", 0),
        "best_score": kb.get("best_score", 0),
        "best_ic": max((_safe_float(item.get("IC", 0)) for item in kb.get("factors", {}).values()), default=0.0),
        "updated_at": live_state.get("updated_at") or kb.get("updated_at", ""),
...
def autoalpha_factor_correlations():
    ...
    try:
        ...
        from autoalpha_v3.factor_research import (

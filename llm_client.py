...
Generation strategy (paper-inspired upgrades):
  ...
  FactorMiner 2026 — Experience-informed guidance:
    ...
    as positive hints; crowded token patterns and high-correlation rejects are
    surfaced as negative constraints.
"""
...
- Keep compute cheap: <= 340 chars and <= 5 total ts_* rolling operators. Avoid nested rolling chains.
...
- Keep compute cheap: <= 340 chars and <= 5 total ts_* rolling operators; no deeply nested ts_* chains.
...
Output ONLY raw JSON:
{
  ...
  "direction": "<reversal | continuation | regime_filter | unknown>",
  "implementation_query": "DSL implementation hint with fields/operators",
  "negative_query": "what failed/correlated structures to avoid",
  "novelty_query": "what should be different from parents"
}
"""
...
def _configured_source_cycle(key: str, default: str) -> list[str]:
    cfg = load_runtime_config()
    sources: list[str] = []
    for raw in str(cfg.get(key, default)).split(","):
        if not raw.strip():
            continue
        source = normalize_source_type(raw.strip())
        if source in {"paper", "llm", "manual"} and source not in sources:
            sources.append(source)
    return sources


def plan_generation_slot(idea_index: int) -> dict[str, Any]:
    """Return deterministic source/mode routing for an absolute idea index."""
    cfg = load_runtime_config()
    ...
    source_cycle = _configured_source_cycle("AUTOALPHA_INSPIRATION_SOURCES", "paper,llm,manual")
    if not source_cycle:
        source_cycle = ["paper", "llm", "manual"]
    exploration_sources = _configured_source_cycle("AUTOALPHA_EXPLORATION_SOURCES", "paper,llm,manual")
    if not exploration_sources:
        exploration_sources = ["paper", "llm", "manual"]
    is_exploration = (idea_index + 1) % explore_every == 0
    is_fresh_blood_slot = (idea_index + 1) % fresh_blood_every == 0
    target_source = (
        exploration_sources[(idea_index // explore_every) % len(exploration_sources)]
        if is_exploration
        else source_cycle[idea_index % len(source_cycle)]
    )
    if is_fresh_blood_slot and not is_exploration and "manual" in exploration_sources:
        target_source = "manual"
    return {
        "target_source": target_source,
        "is_exploration": is_exploration,
        "is_fresh_blood_slot": is_fresh_blood_slot,
...
def generate_idea(
    ...
    slot = plan_generation_slot(idea_index)
    target_source = slot["target_source"]
    is_exploration = bool(slot["is_exploration"])
    is_fresh_blood_slot = bool(slot["is_fresh_blood_slot"])
    if inspirations:
    ...
    print(
        "[inspiration] "
        f"idea_index={idea_index} target={target_source} primary={primary_source} "
        f"selected={sorted(set(source_types))} ids={source_ids[:8]}"
    )

    archetype_options = EXPLORATION_ARCHETYPES if is_exploration else ARCHETYPES
    archetype_key, archetype = search_policy.choose_archetype_ucb(
        archetype_options,
        archetype_stats,
    ...
    query_plan = {
        "mechanism": rag_query,
        "implementation": str((hypothesis_obj or {}).get("implementation_query", "") or rag_query),
        "negative": str((hypothesis_obj or {}).get("negative_query", "") or guidance.get("dominant_failure_mode", "") or ""),
        "novelty": str((hypothesis_obj or {}).get("novelty_query", "") or archetype),
    }
    inspiration_trace: dict[str, Any] = {"mode": "provided" if inspirations else "initial_balanced"}
    if not inspirations:
        inspiration_text, inspiration_rows, inspiration_trace = retrieve_inspiration_context_v3(
            query_plan,
            limit=max(4, context_limit),
            preferred_source=target_source,
            prefer_unused=is_fresh_blood_slot or is_exploration,
        ...
        primary_source = (
            target_source if target_source in source_types
            else (source_types[idea_index % len(source_types)] if source_types else "none")
        )
    ...
    passing_rag = compose_passing_factors_rag(
        ...
        parent_run_ids=[str(p.get("run_id", "")) for p in (parents or [])],
        exploration=is_exploration,
    )
    if passing_rag:
    ...
    failure_memory = compose_failure_memory_context(query_plan, limit=6 if is_exploration else 8)
    if failure_memory:
        sections.append(("failure_memory", failure_memory))
    ...
    if hypothesis_obj:
        ...
        sections.append(
            ...
            f"  Direction: {hypothesis_obj.get('direction', '')}\n"
            f"  Implementation query: {hypothesis_obj.get('implementation_query', '')}\n"
            f"  Negative query: {hypothesis_obj.get('negative_query', '')}\n"
            f"  Novelty query: {hypothesis_obj.get('novelty_query', '')}\n"
    ...
    high_corr_rejections = guidance.get("high_correlation_rejections") or []
    if high_corr_rejections:
        corr_lines = [
            f"  {item.get('run_id','')}: max_abs_corr={float(item.get('max_abs_corr', 0) or 0):.3f} "
            f"closest={item.get('closest_run_id','')} motif={item.get('motif','')} "
            f"formula={str(item.get('formula',''))[:120]}"
            for item in high_corr_rejections[:5]
        ]
        sections.append(
            (
            "correlation_feedback",
            "Recent formulas rejected mainly because they were too correlated with accepted v3 factors. "
            "Avoid these mechanisms/skeletons; choose a different source of signal, not only new windows:\n"
            + "\n".join(corr_lines)
            )
        )
    ...
    mutation_type = search_policy.choose_mutation_type(
        exploration=is_exploration,
        source_type=primary_source,
        recent_failures=f"{failure_summary}\n{failure_memory}",
    )
    sections.append(
        (
        "mode_rules",
        f"This is idea {idea_index + 1} of {total_ideas}. "
        f"Mutation policy: {mutation_type}.\n"
        "Generate ONE novel factor. STRICT RULES:\n"
        f"{mode_rules}"
        "3. Outer smoother window MUST be >= 10 (ts_decay_linear >= 15, ts_ema >= 10).\n"
        "4. Keep compute cheap: <=340 chars, <=5 total ts_* rolling operators, no deeply nested rolling chains.\n"
        "5. Use sigmoid/tanh softening on sub-signals only when it materially helps; avoid decorative wrappers.\n"
        "6. Do NOT copy any formula from the RAG verbatim — new price signal or new baseline.\n"
        "7. Keep at least two novelty changes versus the closest parent: e.g. different baseline + different signal core, or different field pair + different outer smoother.\n"
        "8. Optimize for low factor-library correlation on 2022-2023; 2024 OOS is hidden feedback and must not drive generation.\n"
        "Return JSON only."
        )
    )
    ...
    idea["parent_signature"] = parent_context_signature(parents)
    idea["mutation_type"] = mutation_type
    idea["rag_trace"] = {
        "version": "v3_phase1_phase2",
        "query_plan": query_plan,
        "policy": {
            "archetype": archetype_key,
            "mutation_type": mutation_type,
            "exploration": is_exploration,
            "fresh_blood": is_fresh_blood_slot,
        },
        "inspiration": inspiration_trace,
        "passing_rag": get_last_rag_trace(),
        "context_chars": {
            "inspiration": len(inspiration_text or ""),
            "passing_rag": len(passing_rag or ""),
            "failure_memory": len(failure_memory or ""),
        },
    }
    return idea
